...
#define IS_DOT_DOT(s) \
  (s[0] == '.' && s[1] == '.' && (IS_PATH_SEPARATOR(s[2]) || s[2] == '\0'))
#define IS_PATH_SEPARATOR_U(ch) ((ch == (WCHAR)'\\') || (ch == (WCHAR)'/'))
#define IS_DOT_U(s) \
  (s[0] == (WCHAR)'.' && (IS_PATH_SEPARATOR_U(s[1]) || s[1] == UNICODE_NULL))
#define IS_DOT_DOT_U(s)                        \
  (s[0] == (WCHAR)'.' && s[1] == (WCHAR)'.' && \
   (IS_PATH_SEPARATOR_U(s[2]) || s[2] == UNICODE_NULL))
...
#define RtlStoreUshort(ADDRESS, VALUE)               \
  if ((ULONG_PTR)ADDRESS & SHORT_MASK) {             \
    ((PUCHAR)ADDRESS)[SHORT_LEAST_SIGNIFICANT_BIT] = \
        (UCHAR)(FIRSTBYTE(VALUE));                   \
    ((PUCHAR)ADDRESS)[SHORT_MOST_SIGNIFICANT_BIT] =  \
        (UCHAR)(SECONDBYTE(VALUE));                  \
  } else {                                           \
...
#define RtlStoreUlong(ADDRESS, VALUE)                                          \
  if ((ULONG_PTR)ADDRESS & LONG_MASK) {                                        \
    ...
    ((PUCHAR)ADDRESS)[LONG_3RD_MOST_SIGNIFICANT_BIT] =                         \
        (UCHAR)(SECONDBYTE(VALUE));                                            \
    ((PUCHAR)ADDRESS)[LONG_2ND_MOST_SIGNIFICANT_BIT] =                         \
        (UCHAR)(THIRDBYTE(VALUE));                                             \
...
typedef struct _OBJECT_DIRECTORY_INFORMATION {
    ...
    !defined(___PROCESSOR_NUMBER_DEFINED)
...
#define DECLARE_CONST_UNICODE_STRING(_variablename, _string)             \
  ...
  const UNICODE_STRING _variablename = {sizeof(_string) - sizeof(WCHAR), \
                                        sizeof(_string),                 \
...
#define EXPORT_VA(x)                                               \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT] \
       .VirtualAddress)
#define IMPORT_VA(x)                                               \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT] \
       .VirtualAddress)
#define RELOC_VA(x)                                                   \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC] \
       .VirtualAddress)
#define RESOURCE_VA(x)                                               \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE] \
       .VirtualAddress)
#define EXPORT_SIZE(x) \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].Size)
#define IMPORT_SIZE(x) \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size)
#define RELOC_SIZE(x) \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC].Size)
#define RESOURCE_SIZE(x) \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE].Size)
#define DEBUGDIR_VA(x)                                            \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG] \
       .VirtualAddress)
#define DEBUGDIR_SIZE(x) \
  ((x)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].Size)
#define IS_VALID_HANDLE(hHandle) \
  ((HANDLE)hHandle != (HANDLE)0 && (HANDLE)hHandle != (HANDLE)0xFFFFFFFF)
...
#define TXFS_MODIFY_RM_VALID_FLAGS                                       \
  (TXFS_RM_FLAG_LOGGING_MODE | TXFS_RM_FLAG_RENAME_RM |                  \
   ...
   TXFS_RM_FLAG_LOG_NO_CONTAINER_COUNT_M_In_ | TXFS_RM_FLAG_SHRINK_LOG | \
   TXFS_RM_FLAG_GROW_LOG | TXFS_RM_FLAG_ENFORCE_MINIMUM_SIZE |           \
...
#define FILE_TYPE_NOTIFICATION_FLAG_USAGE_BEG_In_ 0x00000001
#define FILE_TYPE_NOTIFICATION_FLAG_USAGE_END 0x00000002

DEFINE_GUID (FILE_TYPE_NOTIFICATION_GUID_PAGE_FILE, 0x0d0a64a1, 0x38fc, 0x4db8,
             0x9f, 0xe7, 0x3f, 0x43, 0x52, 0xcd, 0x7c, 0x5c);
DEFINE_GUID (FILE_TYPE_NOTIFICATION_GUID_HIBERNATION_FILE, 0xb7624d64, 0xb9a3,
             0x4cf8, 0x80, 0x11, 0x5b, 0x86, 0xc9, 0x40, 0xe7, 0xb7);
DEFINE_GUID (FILE_TYPE_NOTIFICATION_GUID_CRASHDUMP_FILE, 0x9d453eb7, 0xd2a6,
             0x4dbd, 0xa2, 0xe3, 0xfb, 0xd0, 0xed, 0x91, 0x09, 0xa9);
#endif
#endif
...
typedef enum _INTERFACE_TYPE { UnknownInterfaceType = 1 } INTERFACE_TYPE;
...
typedef VOID (*PTIMER_APC_ROUTINE)(_In_ PVOID TimerContext,
...
#define MDL_HASH_INDEX(wch) \
  ((RtlUpcaseUnicodeChar((wch)) - (WCHAR)'A') & MDL_HASH_MASK)
#if !defined(_WINNT_)
...
typedef BOOLEAN (*PRTL_CONFLICT_RANGE_CALLBACK)(_In_ PVOID Context,
...
typedef VOID (*PPS_APC_ROUTINE)(_In_ OPTIONAL PVOID ApcArgument1,
...
typedef NTSTATUS (*PWD_HANDLER)(_In_ WATCHDOG_HANDLER_ACTION Action,
                                _In_ PVOID Context, _In_ _Out_ PULONG DataValue,
...
#define SET_LAST_STATUS(S)         \
  NtCurrentTeb()->LastErrorValue = \
      RtlNtStatusToDosError(NtCurrentTeb()->LastStatusValue = (ULONG)(S))
...
#define LDR_DATA_TABLE_ENTRY_SIZE_WINXP32 \
  FIELD_OFFSET(LDR_DATA_TABLE_ENTRY32, ForwarderLinks)
...
typedef struct _NLS_USER_INFO {
	...
	ULONG        UserLocaleId;
	struct _LUID InteractiveUserLuid;
	UCHAR        InteractiveUserSid[68];
...
typedef ULONG (*PCSR_CALLBACK_ROUTINE)(_In_ _Out_ PCSR_API_MSG ReplyMsg);
...
typedef struct _BASE_SXS_CREATE_ACTIVATION_CONTEXT_MSG {
	...
	UNICODE_STRING                                   AssemblyName;
}     BASE_SXS_CREATE_ACTIVATION_CONTEXT_MSG,
    * PBASE_SXS_CREATE_ACTIVATION_CONTEXT_MSG;
...
typedef BOOLEAN(NTAPI* PDLL_INIT_ROUTINE)(_In_ PVOID DllHandle,
                                          _In_ ULONG Reason,
...
#define PcTeb 0x18
#define RtlGetCurrentProcessId() \
  (HandleToUlong(NtCurrentTeb()->ClientId.UniqueProcess))
#define RtlGetCurrentThreadId() \
...
#define RtlAcquireLockRoutine(L) \
  RtlEnterCriticalSection((PRTL_CRITICAL_SECTION)(L))
...
typedef EXCEPTION_DISPOSITION (*PEXCEPTION_ROUTINE)(
	_In_ struct _EXCEPTION_RECORD* ExceptionRecord, _In_ PVOID EstablisherFrame,
...
typedef NTSTATUS(NTAPI* PRTL_QUERY_REGISTRY_ROUTINE)(
	_In_ PWSTR ValueName, _In_ ULONG ValueType, _In_ PVOID ValueData,
	_In_ ULONG ValueLength, _In_ PVOID Context, _In_ PVOID EntryContext);
...
#define EXCEPTION_CHAIN_END \
  ((struct _EXCEPTION_REGISTRATION_RECORD * POINTER_32) - 1)
...
#define RtlpOwnerAddrSecurityDescriptor(SD)                               \
  (((SD)->Control & SE_SELF_RELATIVE)                                     \
       ? ((((SECURITY_DESCRIPTOR_RELATIVE *)(SD))->Owner == 0)            \
              ? ((PSID)NULL)                                              \
              : (PSID)RtlOffsetToPointer(                                 \
                    (SD), ((SECURITY_DESCRIPTOR_RELATIVE *)(SD))->Owner)) \
       : (PSID)((SD)->Owner))
#define RtlpGroupAddrSecurityDescriptor(SD)                               \
  (((SD)->Control & SE_SELF_RELATIVE)                                     \
       ? ((((SECURITY_DESCRIPTOR_RELATIVE *)(SD))->Group == 0)            \
              ? ((PSID)NULL)                                              \
              : (PSID)RtlOffsetToPointer(                                 \
                    (SD), ((SECURITY_DESCRIPTOR_RELATIVE *)(SD))->Group)) \
       : (PSID)((SD)->Group))
  ...
  ((!((SD)->Control & SE_SACL_PRESENT))                                   \
       ...
       : (((SD)->Control & SE_SELF_RELATIVE)                              \
              ? ((((SECURITY_DESCRIPTOR_RELATIVE *)(SD))->Sacl == 0)      \
                     ? ((PACL)NULL)                                       \
                     : (PACL)RtlOffsetToPointer(                          \
                           (SD),                                          \
                           ((SECURITY_DESCRIPTOR_RELATIVE *)(SD))->Sacl)) \
              : (PACL)((SD)->Sacl)))
  ...
  ((!((SD)->Control & SE_DACL_PRESENT))                                   \
       ...
       : (((SD)->Control & SE_SELF_RELATIVE)                              \
              ? ((((SECURITY_DESCRIPTOR_RELATIVE *)(SD))->Dacl == 0)      \
                     ...
                     : (PACL)RtlOffsetToPointer(                          \
                           (SD),                                          \
                           ((SECURITY_DESCRIPTOR_RELATIVE *)(SD))->Dacl)) \
              : (PACL)((SD)->Dacl)))
...
DEFINE_GUID (Audit_System_SecurityStateChange, 0x0cce9210, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
#define Audit_System_SecurityStateChange_defined
#endif
#endif
#if !defined(INITGUID) || \
    !defined(Audit_System_SecuritySubsystemExtension_defined)
...
DEFINE_GUID (Audit_System_Integrity, 0x0cce9212, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
#define Audit_System_Integrity_defined
#endif
#endif
#if !defined(INITGUID) || !defined(Audit_System_IPSecDriverEvents_defined)
DEFINE_GUID (Audit_System_IPSecDriverEvents, 0x0cce9213, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_System_Others, 0x0cce9214, 0x69ae, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
#define Audit_System_Others_defined
#endif
#endif
#if !defined(INITGUID) || !defined(Audit_Logon_Logon_defined)
DEFINE_GUID (Audit_Logon_Logon, 0x0cce9215, 0x69ae, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon_Logoff, 0x0cce9216, 0x69ae, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon_AccountLockout, 0x0cce9217, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon_IPSecMainMode, 0x0cce9218, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon_IPSecQuickMode, 0x0cce9219, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon_IPSecUserMode, 0x0cce921a, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon_SpecialLogon, 0x0cce921b, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon_Others, 0x0cce921c, 0x69ae, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_ObjectAccess_FileSystem, 0x0cce921d, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_ObjectAccess_Registry, 0x0cce921e, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_ObjectAccess_Kernel, 0x0cce921f, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_ObjectAccess_Sam, 0x0cce9220, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_ObjectAccess_CertificationServices_defined)
DEFINE_GUID (Audit_ObjectAccess_CertificationServices, 0x0cce9221, 0x69ae,
             0x11d9, 0xbe, 0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
#define Audit_ObjectAccess_CertificationServices_defined
#endif
#endif
#if !defined(INITGUID) || \
    !defined(Audit_ObjectAccess_ApplicationGenerated_defined)
...
DEFINE_GUID (Audit_ObjectAccess_Handle, 0x0cce9223, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_ObjectAccess_Share, 0x0cce9224, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_ObjectAccess_FirewallPacketDrops_defined)
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_ObjectAccess_FirewallConnection_defined)
...
DEFINE_GUID (Audit_ObjectAccess_Other, 0x0cce9227, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_PrivilegeUse_Sensitive, 0x0cce9228, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_PrivilegeUse_NonSensitive, 0x0cce9229, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_PrivilegeUse_Others, 0x0cce922a, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_DetailedTracking_ProcessCreation_defined)
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_DetailedTracking_ProcessTermination_defined)
DEFINE_GUID (Audit_DetailedTracking_ProcessTermination, 0x0cce922c, 0x69ae,
             0x11d9, 0xbe, 0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_DetailedTracking_RpcCall, 0x0cce922e, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_PolicyChange_AuditPolicy, 0x0cce922f, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_PolicyChange_AuthenticationPolicy_defined)
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_PolicyChange_AuthorizationPolicy_defined)
...
DEFINE_GUID (Audit_PolicyChange_WfpIPSecPolicy, 0x0cce9233, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_PolicyChange_Others, 0x0cce9234, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_AccountManagement_ComputerAccount_defined)
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_AccountManagement_SecurityGroup_defined)
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_AccountManagement_DistributionGroup_defined)
DEFINE_GUID (Audit_AccountManagement_DistributionGroup, 0x0cce9238, 0x69ae,
             0x11d9, 0xbe, 0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_AccountManagement_ApplicationGroup_defined)
DEFINE_GUID (Audit_AccountManagement_ApplicationGroup, 0x0cce9239, 0x69ae,
             0x11d9, 0xbe, 0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_AccountManagement_Others, 0x0cce923a, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_DSAccess_DSAccess, 0x0cce923b, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_DsAccess_AdAuditChanges, 0x0cce923c, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Ds_Replication, 0x0cce923d, 0x69ae, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Ds_DetailedReplication, 0x0cce923e, 0x69ae, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_AccountLogon_CredentialValidation_defined)
...
DEFINE_GUID (Audit_AccountLogon_Kerberos, 0x0cce9240, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_AccountLogon_Others, 0x0cce9241, 0x69ae, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
#endif
#if !defined(INITGUID) || \
    !defined(Audit_AccountLogon_KerbCredentialValidation_defined)
DEFINE_GUID (Audit_AccountLogon_KerbCredentialValidation, 0x0cce9242, 0x69ae,
             0x11d9, 0xbe, 0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon_NPS, 0x0cce9243, 0x69ae, 0x11d9, 0xbe, 0xd3, 0x50, 0x50,
             0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_System, 0x69979848, 0x797a, 0x11d9, 0xbe, 0xd3, 0x50, 0x50,
             0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_Logon, 0x69979849, 0x797a, 0x11d9, 0xbe, 0xd3, 0x50, 0x50,
             0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_ObjectAccess, 0x6997984a, 0x797a, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_PrivilegeUse, 0x6997984b, 0x797a, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_DetailedTracking, 0x6997984c, 0x797a, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_PolicyChange, 0x6997984d, 0x797a, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_AccountManagement, 0x6997984e, 0x797a, 0x11d9, 0xbe, 0xd3,
             0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_DirectoryServiceAccess, 0x6997984f, 0x797a, 0x11d9, 0xbe,
             0xd3, 0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
DEFINE_GUID (Audit_AccountLogon, 0x69979850, 0x797a, 0x11d9, 0xbe, 0xd3, 0x50,
             0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
NTSTATUS LsaLookupOpenLocalPolicy(_In_ PLSA_OBJECT_ATTRIBUTES ObjectAttributes,
                                  _In_ ACCESS_MASK AccessMask,
                                  _In_ _Out_ PLSA_LOOKUP_HANDLE PolicyHandle);

NTSTATUS LsaLookupClose(_In_ LSA_LOOKUP_HANDLE ObjectHandle);

NTSTATUS LsaLookupTranslateSids(_In_ LSA_LOOKUP_HANDLE PolicyHandle, _In_ ULONG Count,
                                _In_ PSID *                         Sids,
                                _Out_ PLSA_REFERENCED_DOMAIN_LIST * ReferencedDomains,
                                _Out_ PLSA_TRANSLATED_NAME *        Names);
#if (_WIN32_WINNT >= 0x0501)
NTSTATUS LsaLookupTranslateNames(_In_ LSA_LOOKUP_HANDLE PolicyHandle, _In_ ULONG Flags,
                                 _In_ ULONG Count, _In_ PLSA_UNICODE_STRING Names,
                                 _Out_ PLSA_REFERENCED_DOMAIN_LIST * ReferencedDomains,
                                 _Out_ PLSA_TRANSLATED_SID2 *        Sids);
#endif
NTSTATUS LsaLookupGetDomainInfo(_In_ LSA_LOOKUP_HANDLE PolicyHandle,
                                _In_ LSA_LOOKUP_DOMAIN_INFO_CLASS DomainInfoClass,
                                _Out_ PVOID * DomainInfo);

NTSTATUS LsaLookupFreeMemory(_In_ PVOID Buffer);
#endif
...
NTSTATUS
NTAPI
LsaRegisterLogonProcess(_In_ PLSA_STRING LogonProcessName,
                        _Out_ PHANDLE LsaHandle,
                        _Out_ PLSA_OPERATIONAL_MODE SecurityMode);

NTSTATUS
NTAPI
LsaLogonUser(_In_ HANDLE LsaHandle, _In_ PLSA_STRING OriginName,
             ...
             _In_ PTOKEN_SOURCE SourceContext, _Out_ PVOID * ProfileBuffer,
             _Out_ PULONG ProfileBufferLength, _Out_ PLUID LogonId,
             _Out_ PHANDLE Token, _Out_ PQUOTA_LIMITS Quotas,
             _Out_ PNTSTATUS SubStatus);

NTSTATUS
NTAPI
LsaLookupAuthenticationPackage(_In_ HANDLE LsaHandle,
                               _In_ PLSA_STRING PackageName,
                               _Out_ PULONG AuthenticationPackage);

NTSTATUS
NTAPI
LsaFreeReturnBuffer(_In_ PVOID Buffer);

NTSTATUS
NTAPI
LsaCallAuthenticationPackage(_In_ HANDLE LsaHandle,
                             _In_ ULONG AuthenticationPackage,
...
NTSTATUS
NTAPI
LsaDeregisterLogonProcess(_In_ HANDLE LsaHandle);

NTSTATUS
NTAPI
LsaConnectUntrusted(_Out_ PHANDLE LsaHandle);
...
#define POLICY_ALL_ACCESS                                                \
  (STANDARD_RIGHTS_REQUIRED | POLICY_VIEW_LOCAL_INFORMATION |            \
   ...
   POLICY_TRUST_ADM_In_ | POLICY_CREATE_ACCOUNT | POLICY_CREATE_SECRET | \
   POLICY_CREATE_PRIVILEGE | POLICY_SET_DEFAULT_QUOTA_LIMITS |           \
   POLICY_SET_AUDIT_REQUIREMENTS | POLICY_AUDIT_LOG_ADM_In_ |            \
   POLICY_SERVER_ADM_In_ | POLICY_LOOKUP_NAMES)
#define POLICY_READ                                       \
  (STANDARD_RIGHTS_READ | POLICY_VIEW_AUDIT_INFORMATION | \
   POLICY_GET_PRIVATE_INFORMATION)
#define POLICY_WRITE                                                      \
  (STANDARD_RIGHTS_WRITE | POLICY_TRUST_ADM_In_ | POLICY_CREATE_ACCOUNT | \
   POLICY_CREATE_SECRET | POLICY_CREATE_PRIVILEGE |                       \
   POLICY_SET_DEFAULT_QUOTA_LIMITS | POLICY_SET_AUDIT_REQUIREMENTS |      \
   POLICY_AUDIT_LOG_ADM_In_ | POLICY_SERVER_ADMIN)
#define POLICY_EXECUTE                                       \
  (STANDARD_RIGHTS_EXECUTE | POLICY_VIEW_LOCAL_INFORMATION | \
   POLICY_LOOKUP_NAMES)
...
#else
#define TRUST_ATTRIBUTE_QUARANTINED_DOMA_In_ 0x00000004
#endif
#if (_WIN32_WINNT >= 0x0501)
...
NTSTATUS
NTAPI
LsaFreeMemory(_In_ OPTIONAL PVOID Buffer);

NTSTATUS
NTAPI
LsaClose(_In_ LSA_HANDLE ObjectHandle);
#if (_WIN32_WINNT >= 0x0600)
...
NTSTATUS
NTAPI
LsaEnumerateLogonSessions(_Out_ PULONG LogonSessionCount,
                          _Out_ PLUID * LogonSessionList);

NTSTATUS
NTAPI
LsaGetLogonSessionData(_In_ PLUID LogonId,
                       _Out_ PSECURITY_LOGON_SESSION_DATA * ppLogonSessionData);
#endif
NTSTATUS
NTAPI
LsaOpenPolicy(_In_ OPTIONAL PLSA_UNICODE_STRING SystemName,
              _In_ PLSA_OBJECT_ATTRIBUTES ObjectAttributes,
              _In_ ACCESS_MASK DesiredAccess, _Out_ PLSA_HANDLE PolicyHandle);

NTSTATUS
NTAPI
LsaQueryInformationPolicy(_In_ LSA_HANDLE PolicyHandle,
                          _In_ POLICY_INFORMATION_CLASS InformationClass,
                          _Out_ PVOID * Buffer);

NTSTATUS
NTAPI
LsaSetInformationPolicy(_In_ LSA_HANDLE PolicyHandle,
                        _In_ POLICY_INFORMATION_CLASS InformationClass,
                        _In_ PVOID Buffer);
...
LsaQueryDomainInformationPolicy(
	...
	_In_ POLICY_DOMAIN_INFORMATION_CLASS InformationClass, _Out_ PVOID * Buffer);

NTSTATUS
NTAPI
LsaSetDomainInformationPolicy(_In_ LSA_HANDLE PolicyHandle,
                              _In_ POLICY_DOMAIN_INFORMATION_CLASS
                              InformationClass,
                              _In_ OPTIONAL PVOID Buffer);

NTSTATUS
NTAPI
LsaRegisterPolicyChangeNotification(_In_ POLICY_NOTIFICATION_INFORMATION_CLASS
                                    InformationClass,
                                    _In_ HANDLE NotificationEventHandle);

NTSTATUS
NTAPI
LsaUnregisterPolicyChangeNotification(_In_ POLICY_NOTIFICATION_INFORMATION_CLASS
                                      InformationClass,
                                      _In_ HANDLE NotificationEventHandle);
...
LsaEnumerateTrustedDomains(
	_In_ LSA_HANDLE PolicyHandle,
	_In_ _Out_ PLSA_ENUMERATION_HANDLE EnumerationContext, _Out_ PVOID * Buffer,
	_In_ ULONG PreferedMaximumLength, _Out_ PULONG CountReturned);

NTSTATUS
NTAPI
LsaLookupNames(_In_ LSA_HANDLE PolicyHandle, _In_ ULONG Count,
...
NTSTATUS
NTAPI
LsaLookupNames2(_In_ LSA_HANDLE PolicyHandle, _In_ ULONG Flags,
                _In_ ULONG Count, _In_ PLSA_UNICODE_STRING Names,
                _Out_ PLSA_REFERENCED_DOMAIN_LIST * ReferencedDomains,
                _Out_ PLSA_TRANSLATED_SID2 *        Sids);
#endif
NTSTATUS
NTAPI
LsaLookupSids(_In_ LSA_HANDLE PolicyHandle, _In_ ULONG Count, _In_ PSID * Sids,
              _Out_ PLSA_REFERENCED_DOMAIN_LIST *                         ReferencedDomains,
              _Out_ PLSA_TRANSLATED_NAME *                                Names);
...
NTSTATUS
NTAPI
LsaEnumerateAccountsWithUserRight(_In_ LSA_HANDLE PolicyHandle,
...
NTSTATUS
NTAPI
LsaEnumerateAccountRights(_In_ LSA_HANDLE PolicyHandle, _In_ PSID AccountSid,
                          _Out_ PLSA_UNICODE_STRING * UserRights,
                          _Out_ PULONG CountOfRights);

NTSTATUS
NTAPI
LsaAddAccountRights(_In_ LSA_HANDLE PolicyHandle, _In_ PSID AccountSid,
                    _In_ PLSA_UNICODE_STRING UserRights,
                    _In_ ULONG CountOfRights);

NTSTATUS
NTAPI
LsaRemoveAccountRights(_In_ LSA_HANDLE PolicyHandle, _In_ PSID AccountSid,
...
NTSTATUS
NTAPI
LsaOpenTrustedDomainByName(_In_ LSA_HANDLE PolicyHandle,
...
NTSTATUS
NTAPI
LsaQueryTrustedDomainInfo(_In_ LSA_HANDLE PolicyHandle,
...
NTSTATUS
NTAPI
LsaSetTrustedDomainInformation(_In_ LSA_HANDLE PolicyHandle,
                               _In_ PSID TrustedDomainSid,
                               _In_ TRUSTED_INFORMATION_CLASS InformationClass,
                               _In_ PVOID Buffer);
...
NTSTATUS
NTAPI
LsaDeleteTrustedDomain(_In_ LSA_HANDLE PolicyHandle,
                       _In_ PSID TrustedDomainSid);

NTSTATUS
NTAPI
LsaQueryTrustedDomainInfoByName(_In_ LSA_HANDLE PolicyHandle,
                                _In_ PLSA_UNICODE_STRING TrustedDomainName,
                                _In_ TRUSTED_INFORMATION_CLASS InformationClass,
                                _Out_ PVOID * Buffer);
...
NTSTATUS
NTAPI
LsaSetTrustedDomainInfoByName(_In_ LSA_HANDLE PolicyHandle,
                              _In_ PLSA_UNICODE_STRING TrustedDomainName,
                              _In_ TRUSTED_INFORMATION_CLASS InformationClass,
                              _In_ PVOID Buffer);
...
LsaCreateTrustedDomainEx(
	...
	_In_ ACCESS_MASK DesiredAccess, _Out_ PLSA_HANDLE TrustedDomainHandle);
#if (_WIN32_WINNT >= 0x0501)
NTSTATUS
NTAPI
LsaQueryForestTrustInformation(
	_In_ LSA_HANDLE PolicyHandle, _In_ PLSA_UNICODE_STRING TrustedDomainName,
...
LsaSetForestTrustInformation(
	_In_ LSA_HANDLE PolicyHandle, _In_ PLSA_UNICODE_STRING TrustedDomainName,
	_In_ PLSA_FOREST_TRUST_INFORMATION ForestTrustInfo, _In_ BOOLEAN CheckOnly,
	_Out_ PLSA_FOREST_TRUST_COLLISION_INFORMATION * CollisionInfo);
#ifdef TESTING_MATCHING_ROUTINE

NTSTATUS
NTAPI
LsaForestTrustFindMatch(_In_ LSA_HANDLE PolicyHandle, _In_ ULONG Type,
...
NTSTATUS
NTAPI
LsaStorePrivateData(_In_ LSA_HANDLE PolicyHandle,
                    _In_ PLSA_UNICODE_STRING KeyName,
                    _In_ OPTIONAL PLSA_UNICODE_STRING PrivateData);

NTSTATUS
NTAPI
LsaRetrievePrivateData(_In_ LSA_HANDLE PolicyHandle,
                       _In_ PLSA_UNICODE_STRING KeyName,
                       _Out_ PLSA_UNICODE_STRING * PrivateData);

ULONG
NTAPI
LsaNtStatusToWinError(_In_ NTSTATUS Status);
#endif
...
typedef NTSTATUS(NTAPI* PRTL_HEAP_COMMIT_ROUTINE)(
	_In_ PVOID Base, _In_ _Out_ PVOID * CommitAddress,
...
typedef RTL_GENERIC_COMPARE_RESULTS(NTAPI* PRTL_AVL_COMPARE_ROUTINE)(
	_In_ struct _RTL_AVL_TABLE* Table, _In_ PVOID FirstStruct,
	_In_ PVOID SecondStruct);

typedef PVOID(NTAPI* PRTL_AVL_ALLOCATE_ROUTINE)(
	_In_ struct _RTL_AVL_TABLE* Table, _In_ CLONG ByteSize);

typedef VOID(NTAPI* PRTL_AVL_FREE_ROUTINE)(_In_ struct _RTL_AVL_TABLE* Table,
                                           IN PVOID Buffer);

typedef NTSTATUS(NTAPI* PRTL_AVL_MATCH_FUNCTION)(
	_In_ struct _RTL_AVL_TABLE* Table, _In_ PVOID UserData,
	_In_ PVOID MatchData);

typedef RTL_GENERIC_COMPARE_RESULTS(NTAPI* PRTL_AVL_COMPARE_ROUTINE)(
	struct _RTL_AVL_TABLE* Table, PVOID FirstStruct, PVOID SecondStruct);

typedef PVOID(NTAPI* PRTL_AVL_ALLOCATE_ROUTINE)(struct _RTL_AVL_TABLE* Table,
                                                ULONG                  ByteSize);

typedef NTSTATUS(NTAPI* PRTL_AVL_MATCH_FUNCTION)(struct _RTL_AVL_TABLE* Table,
                                                 PVOID                  UserData,
                                                 PVOID                  MatchData);
...
typedef PVOID(NTAPI* PRTL_GENERIC_ALLOCATE_ROUTINE)(
	struct _RTL_GENERIC_TABLE* Table, ULONG ByteSize);

typedef VOID(NTAPI* PRTL_GENERIC_FREE_ROUTINE)(struct _RTL_GENERIC_TABLE* Table,
                                               PVOID                      Buffer);
...
#define HEAP_MAXIMUM_BLOCK_SIZE                                 \
  (USHORT)(((0x10000 << HEAP_GRANULARITY_SHIFT) - PAGE_SIZE) >> \
...
#define XSTATE_MASK_LEGACY_FLOATING_POINT \
  (1i64 << (XSTATE_LEGACY_FLOATING_POINT))
...
C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TickCountMultiplier)
          ==
          0x4
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, InterruptTime)
          ==
          0x8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SystemTime)
          ==
          0x14
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TimeZoneBias)
          ==
          0x20
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ImageNumberLow)
          ==
          0x2c
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ImageNumberHigh)
          ==
          0x2e
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, NtSystemRoot)
          ==
          0x30
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, MaxStackTraceDepth)
          ==
          0x238
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, CryptoExponent)
          ==
          0x23c
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TimeZoneId)
          ==
          0x240
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, LargePageMinimum)
          ==
          0x244
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, Reserved2)
          ==
          0x248
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, NtProductType)
          ==
          0x264
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ProductTypeIsValid)
          ==
          0x268
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, NtMajorVersion)
          ==
          0x26c
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, NtMinorVersion)
          ==
          0x270
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ProcessorFeatures)
          ==
          0x274
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, Reserved1)
          ==
          0x2b4
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, Reserved3)
          ==
          0x2b8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TimeSlip)
          ==
          0x2bc
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, AlternativeArchitecture)
          ==
          0x2c0
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SystemExpirationDate)
          ==
          0x2c8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SuiteMask)
          ==
          0x2d0
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, KdDebuggerEnabled)
          ==
          0x2d4
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, NXSupportPolicy)
          ==
          0x2d5
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ActiveConsoleId)
          ==
          0x2d8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, DismountCount)
          ==
          0x2dC
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ComPlusPackage)
          ==
          0x2e0
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, LastSystemRITEventTickCount)
          ==
          0x2e4
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, NumberOfPhysicalPages)
          ==
          0x2e8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SafeBootMode)
          ==
          0x2ec
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TraceLogging)
          ==
          0x2f0
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TestRetInstruction)
          ==
          0x2f8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SystemCall)
          ==
          0x300
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SystemCallReturn)
          ==
          0x304
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SystemCallPad)
          ==
          0x308
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TickCount)
          ==
          0x320
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TickCountQuad)
          ==
          0x320
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, Cookie)
          ==
          0x330
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ConsoleSessionForegroundProcessId)
          ==
          0x338
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, Wow64SharedInformation)
          ==
          0x340
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, UserModeGlobalLogger)
          ==
          0x380
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ImageFileExecutionOptions)
          ==
          0x3a0
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, LangGenerationCount)
          ==
          0x3a4
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, InterruptTimeBias)
          ==
          0x3b0
         );
...
C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, Reserved5)
          ==
          0x3a8
         );
...
C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, TscQpcBias)
          ==
          0x3b8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ActiveProcessorCount)
          ==
          0x3c0
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, ActiveGroupCount)
          ==
          0x3c4
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, Reserved4)
          ==
          0x3c6
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, AitSamplingValue)
          ==
          0x3c8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, AppCompatFlag)
          ==
          0x3cc
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SystemDllNativeRelocation)
          ==
          0x3d0
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, SystemDllWowRelocation)
          ==
          0x3d8
         );

C_ASSERT (FIELD_OFFSET (KUSER_SHARED_DATA, XState)
          ==
          0x3e0
         );
...
#define RtlInitializeLockRoutine(L)                                    \
  RtlInitializeCriticalSectionAndSpinCount((PRTL_CRITICAL_SECTION)(L), \
                                           (PREALLOCATE_EVENT_MASK | 4000))
#define RtlAcquireLockRoutine(L) \
  RtlEnterCriticalSection((PRTL_CRITICAL_SECTION)(L))
#define RtlReleaseLockRoutine(L) \
  RtlEXITCriticalSection((PRTL_CRITICAL_SECTION)(L))
#define RtlDeleteLockRoutine(L) \
  RtlDeleteCriticalSection((PRTL_CRITICAL_SECTION)(L))
...
* PRTL_OUT_OF_PROCESS_MEMORY_STREAM_DATA;
...
HRESULT
NTAPI
RtlSetMemoryStreamSize(PRTL_MEMORY_STREAM_WITH_VTABLE MemoryStream,
                       ULARGE_INTEGER                 ULargeInteger);

HRESULT
NTAPI
RtlCommitMemoryStream(PRTL_MEMORY_STREAM_WITH_VTABLE MemoryStream,
                      ULONG                          NewStream);
...
NTSTATUS
NTAPI
RtlCopySecurityDescriptor(PSECURITY_DESCRIPTOR SourceDescriptor,
...
#endif
VOID NTAPI RtlInitializeHandleTable(_In_ ULONG MaximumNumberOfHandles,
                                    _In_ ULONG SizeOfHandleTableEntry,
                                    _Out_ PRTL_HANDLE_TABLE HandleTable);

NTSTATUS
NTAPI
RtlDestroyHandleTable(_In_ _Out_ PRTL_HANDLE_TABLE HandleTable);

PRTL_HANDLE_TABLE_ENTRY
NTAPI
RtlAllocateHandle(_In_ PRTL_HANDLE_TABLE HandleTable,
                  _Out_ OPTIONAL PULONG HandleIndex);

BOOLEAN
NTAPI
RtlFreeHandle(_In_ PRTL_HANDLE_TABLE HandleTable,
              _In_ PRTL_HANDLE_TABLE_ENTRY Handle);

BOOLEAN
NTAPI
RtlIsValidHandle(_In_ PRTL_HANDLE_TABLE HandleTable,
                 _In_ PRTL_HANDLE_TABLE_ENTRY Handle);

BOOLEAN
NTAPI
RtlIsValidIndexHandle(_In_ PRTL_HANDLE_TABLE HandleTable,
...
NTSTATUS
NTAPI
RtlDestroyAtomTable(_In_ PVOID AtomTableHandle);
...
NTSTATUS
NTAPI
RtlPinAtomInAtomTable(_In_ PVOID AtomTableHandle, _In_ RTL_ATOM Atom);
...
NTSTATUS
NTAPI
RtlQueryAtomsInAtomTable(_In_ PVOID AtomTableHandle,
                         _In_ ULONG MaximumNumberOfAtoms,
                         _Out_ PULONG NumberOfAtoms, _Out_ PRTL_ATOM Atoms);

BOOLEAN
NTAPI
RtlGetIntegerAtom(_In_ PWSTR AtomName, _Out_ OPTIONAL PUSHORT IntegerAtom);
...
typedef struct DECLSPEC_ALIGN (16) _SLIST_ENTRY { PSLIST_ENTRY Next; } SLIST_ENTRY;
#pragma warning(pop)
...
#endif
#endif
PSLIST_ENTRY __fastcall RtlInterlockedPushListSList(_In_ PSLIST_HEADER ListHead,
...
VOID NTAPI RtlAssert(_In_ PVOID VoidFailedAssertion, _In_ PVOID VoidFileName,
                     _In_ ULONG LineNumber, _In_ OPTIONAL PSTR MutableMessage);

VOID NTAPI RtlInitializeGenericTableAvl(
	PRTL_AVL_TABLE            Table, PRTL_AVL_COMPARE_ROUTINE CompareRoutine,
	PRTL_AVL_ALLOCATE_ROUTINE AllocateRoutine,
	PRTL_AVL_FREE_ROUTINE     FreeRoutine, PVOID TableContext);

PVOID
NTAPI
RtlInsertElementGenericTableAvl(PRTL_AVL_TABLE Table, PVOID Buffer,
                                ULONG          BufferSize, PBOOLEAN NewElement OPTIONAL);

PVOID
NTAPI
RtlInsertElementGenericTableFullAvl(PRTL_AVL_TABLE Table, PVOID Buffer,
                                    ULONG          BufferSize,
...
PVOID
NTAPI
RtlLookupElementGenericTableFullAvl(PRTL_AVL_TABLE              Table, PVOID Buffer,
...
PVOID
NTAPI
RtlEnumerateGenericTableWithoutSplayingAvl(PRTL_AVL_TABLE Table,
                                           PVOID*         RestartKey);

PVOID
NTAPI
RtlEnumerateGenericTableLikeADirectory(
	_In_ PRTL_AVL_TABLE Table, _In_ PRTL_AVL_MATCH_FUNCTION MatchFunction,
	_In_ PVOID MatchData, _In_ ULONG NextFlag, _In_ _Out_ PVOID * RestartKey,
	_In_ _Out_ PULONG DeleteCount, _In_ _Out_ PVOID Buffer);
...
PVOID
NTAPI
RtlInsertElementGenericTable(PRTL_GENERIC_TABLE Table, PVOID Buffer,
                             ULONG              BufferSize, PBOOLEAN NewElement OPTIONAL);

PVOID
NTAPI
RtlInsertElementGenericTableFull(PRTL_GENERIC_TABLE  Table, PVOID Buffer,
                                 ULONG               BufferSize, PBOOLEAN NewElement OPTIONAL,
                                 PVOID               NodeOrParent,
                                 TABLE_SEARCH_RESULT SearchResult);
...
PVOID
NTAPI
RtlLookupElementGenericTableFull(PRTL_GENERIC_TABLE          Table, PVOID Buffer,
                                 _Out_ PVOID *               NodeOrParent,
                                 _Out_ TABLE_SEARCH_RESULT * SearchResult);
...
PVOID
NTAPI
RtlEnumerateGenericTableWithoutSplaying(PRTL_GENERIC_TABLE Table,
                                        PVOID*             RestartKey);
...
PVOID
NTAPI
RtlCreateHeap(_In_ ULONG Flags, _In_ PVOID HeapBase OPTIONAL,
              _In_ SIZE_T ReserveSize OPTIONAL, _In_ SIZE_T CommitSize OPTIONAL,
...
BOOLEAN
NTAPI
RtlFreeHeap(_In_ PVOID HeapHandle, _In_ OPTIONAL ULONG Flags,
...
ULONG
NTAPI
RtlCaptureStackContext(_Out_ PULONG_PTR Callers,
                       _Out_ PRTL_STACK_CONTEXT Context, _In_ ULONG Limit);
...
NTSTATUS
NTAPI
RtlOpenCurrentUser(_In_ ULONG DesiredAccess, _Out_ PHANDLE CurrentUserKey);

NTSTATUS
NTAPI
RtlQueryRegistryValues(_In_ ULONG RelativeTo, _In_ PCWSTR Path,
                       _In_ PRTL_QUERY_REGISTRY_TABLE QueryTable,
                       _In_ PVOID Context, _In_ PVOID Environment OPTIONAL);

NTSTATUS
NTAPI
RtlWriteRegistryValue(_In_ ULONG RelativeTo, _In_ PCWSTR Path,
                      _In_ PCWSTR ValueName, _In_ ULONG ValueType,
                      _In_ PVOID ValueData, _In_ ULONG ValueLength);

NTSTATUS
NTAPI
RtlDeleteRegistryValue(_In_ ULONG RelativeTo, _In_ PCWSTR Path,
...
NTSTATUS
NTAPI
RtlCheckRegistryKey(_In_ ULONG RelativeTo, _In_ PWSTR Path);
...
RtlReAllocateHeap(_In_ PVOID HeapHandle, _In_ ULONG Flags,
                  _In_ PVOID BaseAddress, _In_ SIZE_T Size);

BOOLEAN
NTAPI
RtlGetUserInfoHeap(_In_ PVOID HeapHandle, _In_ ULONG Flags,
                   _In_ PVOID BaseAddress, _Out_ OPTIONAL PVOID * UserValue,
...
RtlCreateTagHeap(_In_ PVOID HeapHandle, _In_ ULONG Flags,
                 _In_ OPTIONAL PWSTR TagPrefix, _In_ PWSTR TagNames);
...
NTSTATUS
NTAPI
RtlExtendHeap(_In_ PVOID HeapHandle, _In_ ULONG Flags, _In_ PVOID Base,
              _In_ SIZE_T Size);
...
NTSTATUS
NTAPI
RtlUsageHeap(_In_ PVOID HeapHandle, _In_ ULONG Flags,
             _In_ _Out_ PRTL_HEAP_USAGE Usage);

NTSTATUS
NTAPI
RtlWalkHeap(_In_ PVOID HeapHandle, _In_ _Out_ PRTL_HEAP_WALK_ENTRY Entry);
...
ULONG
NTAPI
RtlMultipleAllocateHeap(_In_ PVOID HeapHandle, _In_ ULONG Flags,
                        _In_ SIZE_T Size, _In_ ULONG Count, _Out_ PVOID * Array);

ULONG
NTAPI
RtlMultipleFreeHeap(_In_ PVOID HeapHandle, _In_ ULONG Flags, _In_ ULONG Count,
                    _In_ PVOID * Array);
...
NTSTATUS
NTAPI
RtlCreateMemoryZone(_Out_ PVOID * MemoryZone, _In_ SIZE_T InitialSize,
                    ULONG         Flags);

NTSTATUS
NTAPI
RtlDestroyMemoryZone(_In_ PVOID MemoryZone);

NTSTATUS
NTAPI
RtlAllocateMemoryZone(_In_ PVOID MemoryZone, _In_ SIZE_T BlockSize,
                      _Out_ PVOID * Block);

NTSTATUS
NTAPI
RtlResetMemoryZone(_In_ PVOID MemoryZone);

NTSTATUS
NTAPI
RtlLockMemoryZone(_In_ PVOID MemoryZone);

NTSTATUS
NTAPI
RtlUnlockMemoryZone(_In_ PVOID MemoryZone);
#endif
#if (NTDDI_VERSION >= NTDDI_VISTA)
NTSTATUS
NTAPI
RtlCreateMemoryBlockLookaside(_Out_ PVOID * MemoryBlockLookaside,
                              _In_ ULONG Flags, _In_ ULONG InitialSize,
                              _In_ ULONG MinimumBlockSize,
                              _In_ ULONG MaximumBlockSize);

NTSTATUS
NTAPI
RtlDestroyMemoryBlockLookaside(_In_ PVOID MemoryBlockLookaside);

NTSTATUS
NTAPI
RtlAllocateMemoryBlockLookaside(_In_ PVOID MemoryBlockLookaside,
                                _In_ ULONG BlockSize, _Out_ PVOID * Block);

NTSYSAPI
NTSTATUS
NTAPI RtlFreeMemoryBlockLookaside(_In_ PVOID MemoryBlockLookaside, _In_ PVOID Block);

NTSTATUS
NTAPI
RtlExtendMemoryBlockLookaside(_In_ PVOID MemoryBlockLookaside,
                              _In_ ULONG Increment);

NTSTATUS
NTAPI
RtlResetMemoryBlockLookaside(_In_ PVOID MemoryBlockLookaside);

NTSTATUS
NTAPI
RtlLockMemoryBlockLookaside(_In_ PVOID MemoryBlockLookaside);

NTSTATUS
NTAPI
RtlUnlockMemoryBlockLookaside(_In_ PVOID MemoryBlockLookaside);
#endif
HANDLE
NTAPI
...
LOGICAL
NTAPI
RtlSetCurrentTransaction(_In_ HANDLE TransactionHandle);

PRTL_DEBUG_INFORMATION
NTAPI
RtlCreateQueryDebugBuffer(_In_ OPTIONAL ULONG MaximumCommit,
                          _In_ BOOLEAN UseEventPair);

NTSTATUS
NTAPI
RtlDestroyQueryDebugBuffer(_In_ PRTL_DEBUG_INFORMATION Buffer);
...
NTSTATUS RtlComputeImportTableHash(_In_ HANDLE hFile, _Out_ PCHAR Hash,
...
NTSTATUS
NTAPI
RtlIntegerToUnicode(_In_ ULONG Value, _In_ ULONG Base OPTIONAL,
                    _In_ LONG OutputLength, _Out_ PWSTR String);

NTSTATUS
NTAPI
RtlLargeIntegerToChar(PLARGE_INTEGER Value, ULONG Base OPTIONAL,
                      LONG           OutputLength, PSZ String);

NTSTATUS
NTAPI
RtlLargeIntegerToUnicode(_In_ PLARGE_INTEGER Value, _In_ ULONG Base OPTIONAL,
                         _In_ LONG OutputLength, _Out_ PWSTR String);
...
NTSTATUS
NTAPI
RtlIpv4AddressToStringExA (_In_
	const struct in_addr* Address, _In_ USHORT Port,
...
NTSTATUS
NTAPI
RtlIpv6AddressToStringExA (_In_
	const struct in6_addr* Address,
                          _In_ ULONG ScopeId, _In_ USHORT Port,
                          _Out_ PSTR AddressString,
                          _In_ _Out_ PULONG AddressStringLength
);
...
PWSTR
NTAPI
RtlIpv4AddressToStringW (_In_
	const struct in_addr* Addr, _Out_ PWSTR S
);

PWSTR
NTAPI
RtlIpv6AddressToStringW (_In_
	const struct in6_addr* Addr, _Out_ PWSTR S
);

NTSTATUS
NTAPI
RtlIpv4AddressToStringExW (_In_
	const struct in_addr* Address, _In_ USHORT Port,
...
NTSTATUS
NTAPI
RtlIpv6AddressToStringExW (_In_
	const struct in6_addr* Address,
                          _In_ ULONG ScopeId, _In_ USHORT Port,
                          _Out_ PWSTR AddressString,
                          _In_ _Out_ PULONG AddressStringLength
...
NTSTATUS
NTAPI
RtlIpv4StringToAddressA(_In_ PCSTR S, _In_ BOOLEAN Strict,
                        _Out_ PCSTR * Terminator, _Out_ struct in_addr* Addr);

NTSTATUS
NTAPI
RtlIpv6StringToAddressA(_In_ PCSTR S, _Out_ PCSTR * Terminator,
                        _Out_ struct in6_addr*      Addr);

NTSTATUS
NTAPI
RtlIpv4StringToAddressExA(_In_ PCSTR AddressString, _In_ BOOLEAN Strict,
                          _Out_ struct in_addr* Address, _Out_ PUSHORT Port);

NTSTATUS
NTAPI
RtlIpv6StringToAddressExA(_In_ PCSTR AddressString,
                          _Out_ struct in6_addr* Address, _Out_ PULONG ScopeId,
                          _Out_ PUSHORT Port);

NTSTATUS
NTAPI
RtlIpv4StringToAddressW(_In_ PCWSTR S, _In_ BOOLEAN Strict,
                        _Out_ LPCWSTR * Terminator, _Out_ struct in_addr* Addr);

NTSTATUS
NTAPI
RtlIpv6StringToAddressW(_In_ PCWSTR S, _Out_ PCWSTR * Terminator,
                        _Out_ struct in6_addr*        Addr);

NTSTATUS
NTAPI
RtlIpv4StringToAddressExW(_In_ PCWSTR AddressString, _In_ BOOLEAN Strict,
                          _Out_ struct in_addr* Address, _Out_ PUSHORT Port);

NTSTATUS
NTAPI
RtlIpv6StringToAddressExW(_In_ PCWSTR AddressString,
                          _Out_ struct in6_addr* Address, _Out_ PULONG ScopeId,
                          _Out_ PUSHORT Port);
...
NTSTATUS
NTAPI
RtlInt64ToUnicodeString(_In_ ULONGLONG Value, _In_ ULONG Base OPTIONAL,
...
NTSTATUS
NTAPI
RtlInitAnsiStringEx(_Out_ PANSI_STRING DestinationString,
                    _In_ PCSZ SourceString OPTIONAL);

BOOLEAN
NTAPI
RtlCreateUnicodeString(_Out_ PUNICODE_STRING DestinationString,
                       _In_ PCWSTR SourceString);

BOOLEAN
NTAPI
RtlEqualDomainName(_In_ PCUNICODE_STRING String1,
                   _In_ PCUNICODE_STRING String2);

BOOLEAN
NTAPI
RtlEqualComputerName(_In_ PCUNICODE_STRING String1,
                     _In_ PCUNICODE_STRING String2);

NTSTATUS RtlDnsHostNameToComputerName(_Out_ PUNICODE_STRING ComputerNameString,
                                      _In_ PCUNICODE_STRING DnsHostNameString,
                                      _In_ BOOLEAN AllocateComputerNameString);

BOOLEAN
NTAPI
RtlCreateUnicodeStringFromAsciiz(_Out_ PUNICODE_STRING DestinationString,
...
LONG NTAPI RtlCompareString(const STRING* String1, const STRING* String2,
                            BOOLEAN       CaseInSensitive);

BOOLEAN
NTAPI
RtlEqualString(const STRING* String1, const STRING* String2,
               BOOLEAN       CaseInSensitive);

BOOLEAN
NTAPI
RtlPrefixString(const STRING* String1, const STRING* String2,
                BOOLEAN       CaseInSensitive);
...
NTSTATUS
NTAPI
RtlUpcaseUnicodeStringToAnsiString(PANSI_STRING     DestinationString,
                                   PCUNICODE_STRING SourceString,
                                   BOOLEAN          AllocateDestinationString);
...
NTSTATUS
NTAPI
RtlUpcaseUnicodeStringToOemString(POEM_STRING      DestinationString,
                                  PCUNICODE_STRING SourceString,
                                  BOOLEAN          AllocateDestinationString);

NTSTATUS
NTAPI
RtlOemStringToCountedUnicodeString(PUNICODE_STRING DestinationString,
                                   PCOEM_STRING    SourceString,
                                   BOOLEAN         AllocateDestinationString);

NTSTATUS
NTAPI
RtlUnicodeStringToCountedOemString(POEM_STRING      DestinationString,
                                   PCUNICODE_STRING SourceString,
                                   BOOLEAN          AllocateDestinationString);

NTSTATUS
NTAPI
RtlUpcaseUnicodeStringToCountedOemString(POEM_STRING      DestinationString,
                                         PCUNICODE_STRING SourceString,
                                         BOOLEAN          AllocateDestinationString);

LONG NTAPI RtlCompareUnicodeString(PCUNICODE_STRING String1,
...
NTSTATUS
NTAPI
RtlHashUnicodeString(_In_ const UNICODE_STRING * String,
                     _In_ BOOLEAN CaseInSensitive, _In_ ULONG HashAlgorithm,
                     _Out_ PULONG HashValue);

NTSTATUS
NTAPI
RtlValidateUnicodeString(_In_ ULONG Flags, _In_ const UNICODE_STRING * String);

NTSTATUS
NTAPI
RtlDuplicateUnicodeString(_In_ ULONG Flags, _In_ const UNICODE_STRING * StringIn,
                          _Out_ UNICODE_STRING *                        StringOut);

BOOLEAN
NTAPI
RtlPrefixUnicodeString(_In_ PCUNICODE_STRING String1,
                       _In_ PCUNICODE_STRING String2,
                       _In_ BOOLEAN CaseInSensitive);

NTSTATUS
NTAPI
RtlUpcaseUnicodeString(PUNICODE_STRING  DestinationString,
                       PCUNICODE_STRING SourceString,
                       BOOLEAN          AllocateDestinationString);

NTSTATUS
NTAPI
RtlFindCharInUnicodeString(_In_ ULONG Flags,
...
NTSTATUS
NTAPI
RtlMultiByteToUnicodeSize(PULONG BytesInUnicodeString, PCSTR MultiByteString,
                          ULONG  BytesInMultiByteString);

NTSTATUS
NTAPI
RtlUnicodeToMultiByteSize(_Out_ PULONG BytesInMultiByteString,
                          _In_ PWCH UnicodeString,
                          _In_ ULONG BytesInUnicodeString);

NTSTATUS
NTAPI
RtlUnicodeToMultiByteN(_Out_ PCHAR MultiByteString,
...
NTSTATUS
NTAPI
RtlUpcaseUnicodeToMultiByteN(_Out_ PCHAR MultiByteString,
                             _In_ ULONG MaxBytesInMultiByteString,
                             _Out_ OPTIONAL PULONG BytesInMultiByteString,
                             _In_ PWCH UnicodeString,
                             _In_ ULONG BytesInUnicodeString);
...
NTSTATUS
NTAPI
RtlOemToUnicodeN(_Out_ PWSTR UnicodeString, _In_ ULONG MaxBytesInUnicodeString,
                 _Out_ OPTIONAL PULONG BytesInUnicodeString, _In_ PCH OemString,
                 _In_ ULONG BytesInOemString);

NTSTATUS
NTAPI
RtlUnicodeToOemN(_Out_ PCHAR OemString, _In_ ULONG MaxBytesInOemString,
                 _Out_ OPTIONAL PULONG BytesInOemString,
                 _In_ PWCH UnicodeString, _In_ ULONG BytesInUnicodeString);

NTSTATUS
NTAPI
RtlUpcaseUnicodeToOemN(_Out_ PCHAR OemString, _In_ ULONG MaxBytesInOemString,
                       _Out_ OPTIONAL PULONG BytesInOemString,
                       _In_ PWCH UnicodeString,
                       _In_ ULONG BytesInUnicodeString);
...
RtlConsoleMultiByteToUnicodeN(
	_Out_ PWCH UnicodeString, _In_ ULONG MaxBytesInUnicodeString,
	_Out_ OPTIONAL PULONG BytesInUnicodeString OPTIONAL,
	_In_ PCH MultiByteString, _In_ ULONG BytesInMultiByteString,
	_Out_ PULONG pdwSpecialChar);

BOOLEAN
NTAPI
RtlIsTextUnicode(_In_ CONST VOID * Buffer, _In_ ULONG Size,
...
VOID NTAPI RtlGenerate8dot3Name(_In_ PUNICODE_STRING Name,
...
BOOLEAN
NTAPI
RtlIsNameLegalDOS8Dot3(_In_ PUNICODE_STRING Name,
                       _In_ _Out_ POEM_STRING OemName OPTIONAL,
                       _In_ _Out_ PBOOLEAN NameContainsSpaces OPTIONAL);

VOID NTAPI RtlInitializeContext(HANDLE Process, PCONTEXT Context,
                                PVOID  Parameter, PVOID  InitialPc,
                                PVOID  InitialSp);

NTSTATUS
NTAPI
RtlRemoteCall(HANDLE  Process, HANDLE           Thread, PVOID      CallSite,
              ULONG   ArgumentCount, PULONG_PTR Arguments, BOOLEAN PassContext,
...
NTSTATUS
STDAPIVCALLTYPE
RtlSetProcessIsCritical(_In_ BOOLEAN NewValue, _Out_ PBOOLEAN OldValue OPTIONAL,
                        _In_ BOOLEAN CheckFlag);

NTSTATUS
STDAPIVCALLTYPE
RtlSetThreadIsCritical(_In_ BOOLEAN NewValue, _Out_ PBOOLEAN OldValue OPTIONAL,
                       _In_ BOOLEAN CheckFlag);
...
NTSTATUS
NTAPI
RtlSetEnvironmentVariable(PVOID*           Environment, PCUNICODE_STRING Name,
                          PCUNICODE_STRING Value);

ULONG RtlIsDosDeviceName_U(_In_ PWSTR DosFileName);

NTSTATUS
NTAPI
RtlQueryEnvironmentVariable_U(PVOID           Environment, PCUNICODE_STRING Name,
                              PUNICODE_STRING Value);

NTSTATUS
NTAPI
RtlExpandEnvironmentStrings_U(_In_ PVOID Environment OPTIONAL,
...
BOOLEAN
NTAPI
PfxInsertPrefix(PPREFIX_TABLE       PrefixTable, PSTRING Prefix,
                PPREFIX_TABLE_ENTRY PrefixTableEntry);

VOID NTAPI PfxRemovePrefix(PPREFIX_TABLE       PrefixTable,
                           PPREFIX_TABLE_ENTRY PrefixTableEntry);
...
NTSTATUS
NTAPI
RtlDescribeChunk(_In_ USHORT CompressionFormat,
                 _In_ _Out_ PUCHAR * CompressedBuffer,
                 _In_ PUCHAR EndOfCompressedBufferPlus1,
                 _Out_ PUCHAR * ChunkBuffer, _Out_ PULONG ChunkSize);

NTSTATUS
NTAPI
RtlReserveChunk(_In_ USHORT CompressionFormat,
                _In_ _Out_ PUCHAR * CompressedBuffer,
                _In_ PUCHAR EndOfCompressedBufferPlus1,
                _Out_ PUCHAR * ChunkBuffer, _In_ ULONG ChunkSize);

NTSTATUS
NTAPI
RtlDecompressChunks(_Out_ PUCHAR UncompressedBuffer,
                    _In_ ULONG UncompressedBufferSize,
                    _In_ PUCHAR CompressedBuffer,
                    _In_ ULONG CompressedBufferSize, _In_ PUCHAR CompressedTail,
                    _In_ ULONG CompressedTailSize,
                    _In_ PCOMPRESSED_DATA_INFO CompressedDataInfo);

NTSTATUS
NTAPI
RtlCompressChunks(_In_ PUCHAR UncompressedBuffer,
                  _In_ ULONG UncompressedBufferSize,
                  _Out_ PUCHAR CompressedBuffer,
                  _In_ ULONG CompressedBufferSize,
                  _In_ _Out_ PCOMPRESSED_DATA_INFO CompressedDataInfo,
                  _In_ ULONG CompressedDataInfoLength, _In_ PVOID WorkSpace);
...
NTSTATUS
NTAPI
RtlCreateUserProcess(PUNICODE_STRING               NtImagePathName, ULONG Attributes,
                     PRTL_USER_PROCESS_PARAMETERS  ProcessParameters,
                     PSECURITY_DESCRIPTOR          ProcessSecurityDescriptor,
                     PSECURITY_DESCRIPTOR          ThreadSecurityDescriptor,
                     HANDLE                        ParentProcess, BOOLEAN InheritHandles,
...
RtlCreateUserThread(HANDLE               Process,
                    ...
                    PUSER_THREAD_START_ROUTINE StartAddress, PVOID Parameter,
                    PHANDLE                    Thread, PCLIENT_ID  ClientId);
...
NTSTATUS
NTAPI
RtlImageNtHeaderEx(ULONG                     Flags, PVOID Base, ULONG64 Size,
...
NTAPI
RtlSectionTableFromVirtualAddress(_In_ PIMAGE_NT_HEADERS NtHeaders,
                                  _In_ PVOID BaseOfImage,
                                  _In_ ULONG VirtualAddress);

NTSTATUS
NTAPI
RtlImageDirectoryEntryToData(PVOID  BaseOfImage, BOOLEAN   MappedAsImage,
                             USHORT DirectoryEntry, PULONG Size);

PVOID RtlImageDirectoryEntryToData32(_In_ PVOID Base, _In_ BOOLEAN MappedAsImage,
...
VOID NTAPI RtlCopyMemoryNonTemporal(VOID UNALIGNED *       Destination,
                                    CONST VOID UNALIGNED * Source,
                                    SIZE_T                 Length);

NTSTATUS
NTAPI
RtlCopyMappedMemory(_In_ LPVOID Destination, _In_ LPVOID Source,
                    _In_ SIZE_T Length);

VOID __fastcall RtlPrefetchMemoryNonTemporal(_In_ PVOID Source,
                                             _In_ SIZE_T Length);
...
VOID NTAPI RtlFillMemory2(PVOID Destination, SIZE_T Length, INT Pattern);

VOID NTAPI RtlFillMemoryUlong(PVOID Destination, SIZE_T Length, ULONG Pattern);

VOID NTAPI RtlFillMemoryUlonglong(PVOID     Destination, SIZE_T Length,
                                  ULONGLONG Pattern);

VOID NTAPI RtlInitializeExceptionLog(_In_ ULONG Entries);

LONG NTAPI
RtlUnhandledExceptionFilter (_In_
	struct _EXCEPTION_POINTERS* ExceptionInfo
);

LONG NTAPI RtlUnhandledExceptionFilter2 (_In_ struct _EXCEPTION_POINTERS * ExceptionInfo, _In_ PCSTR Function);

VOID NTAPI DbgUserBreakPoint(VOID);

VOID NTAPI DbgBreakPointWithStatus(_In_ ULONG Status);

ULONG DbgPrintEx(_In_ ULONG ComponentId, _In_ ULONG Level, _In_ PCH Format, ...);

ULONG
NTAPI
vDbgPrintEx(_In_ ULONG ComponentId, _In_ ULONG Level, _In_ PCH Format,
            _In_ va_list arglist);

ULONG
NTAPI
vDbgPrintExWithPrefix(_In_ PCH Prefix, _In_ ULONG ComponentId, _In_ ULONG Level,
                      _In_ PCH Format, _In_ va_list arglist);

ULONG DbgPrintReturnControlC(_In_ PCHAR Format, ...);
...
NTSTATUS
NTAPI
DbgSetDebugFilterState(_In_ ULONG ComponentId, _In_ ULONG Level,
                       _In_ BOOLEAN State);

ULONG
NTAPI
DbgPrompt(_In_ PCH Prompt, _Out_ PCH Response, _In_ ULONG Length);

VOID NTAPI DbgLoadImageSymbols(_In_ PSTRING FileName, _In_ PVOID ImageBase,
                               _In_ ULONG_PTR ProcessId);

VOID NTAPI DbgUnLoadImageSymbols(_In_ PSTRING FileName, _In_ PVOID ImageBase,
                                 _In_ ULONG_PTR ProcessId);

VOID NTAPI DbgCommandString(_In_ PCH Name, _In_ PCH Command);

BOOLEAN
NTAPI
RtlCutoverTimeToSystemTime(PTIME_FIELDS   CutoverTime, PLARGE_INTEGER SystemTime,
                           PLARGE_INTEGER CurrentSystemTime, BOOLEAN  ThisYear);

NTSTATUS
NTAPI
RtlSystemTimeToLocalTime(_In_ PLARGE_INTEGER SystemTime,
                         _Out_ PLARGE_INTEGER LocalTime);

NTSTATUS
NTAPI
RtlLocalTimeToSystemTime(_In_ PLARGE_INTEGER LocalTime,
                         _Out_ PLARGE_INTEGER SystemTime);

VOID NTAPI RtlTimeToElapsedTimeFields(_In_ PLARGE_INTEGER Time,
...
NTSTATUS
NTAPI
RtlSetActiveTimeBias(_In_ LONG ActiveBias);

VOID NTAPI RtlInitializeBitMap(PRTL_BITMAP BitMapHeader, PULONG BitMapBuffer,
                               ULONG       SizeOfBitMap);
...
ULONG
NTAPI
RtlFindClearBitsAndSet(PRTL_BITMAP BitMapHeader, ULONG NumberToFind,
                       ULONG       HintIndex);

ULONG
NTAPI
RtlFindSetBitsAndClear(PRTL_BITMAP BitMapHeader, ULONG NumberToFind,
                       ULONG       HintIndex);

VOID NTAPI RtlClearBits(PRTL_BITMAP BitMapHeader, ULONG StartingIndex,
                        ULONG       NumberToClear);

VOID NTAPI RtlSetBits(PRTL_BITMAP BitMapHeader, ULONG StartingIndex,
                      ULONG       NumberToSet);

ULONG
NTAPI
RtlFindClearRuns(PRTL_BITMAP BitMapHeader, PRTL_BITMAP_RUN RunArray,
                 ULONG       SizeOfRunArray, BOOLEAN       LocateLongestRuns);
...
RtlAllocateAndInitializeSid(_In_ PSID_IDENTIFIER_AUTHORITY IdentifierAuthority,
                            _In_ UCHAR SubAuthorityCount,
                            _In_ ULONG SubAuthority0, _In_ ULONG SubAuthority1,
                            _In_ ULONG SubAuthority2, _In_ ULONG SubAuthority3,
                            _In_ ULONG SubAuthority4, _In_ ULONG SubAuthority5,
                            _In_ ULONG SubAuthority6, _In_ ULONG SubAuthority7,
                            _Out_ PSID * Sid);
...
NTSTATUS
NTAPI
RtlCopySidAndAttributesArray(ULONG               ArrayLength, PSID_AND_ATTRIBUTES Source,
                             ULONG               TargetSidBufferSize,
                             PSID_AND_ATTRIBUTES TargetArrayElement,
                             PSID                TargetSid, PSID* NextTargetSid,
...
NTSTATUS
NTAPI
RtlConvertSidToUnicodeString(PUNICODE_STRING UnicodeString, PSID Sid,
                             BOOLEAN         AllocateDestinationString);

VOID NTAPI RtlCopyLuid(PLUID DestinationLuid, PLUID SourceLuid);

VOID NTAPI RtlCopyLuidAndAttributesArray(ULONG                ArrayLength,
...
VOID NTAPI RtlMapGenericMask(PACCESS_MASK     AccessMask,
...
NTSTATUS
NTAPI
RtlQueryInformationAcl(PACL                  Acl, PVOID AclInformation,
                       ULONG                 AclInformationLength,
                       ACL_INFORMATION_CLASS AclInformationClass);

NTSTATUS
NTAPI
RtlSetInformationAcl(PACL                  Acl, PVOID AclInformation, ULONG AclInformationLength,
                     ACL_INFORMATION_CLASS AclInformationClass);

NTSTATUS
NTAPI
RtlAddAce(PACL  Acl, ULONG AceRevision, ULONG StartingAceIndex, PVOID AceList,
          ULONG AceListLength);
...
RtlSetOwnerSecurityDescriptor(
	...
	_In_ OPTIONAL PSID Owner, _In_ OPTIONAL BOOLEAN OwnerDefaulted);
...
NTSTATUS
NTAPI
RtlAddAccessAllowedAce(PACL Acl, ULONG AceRevision, ACCESS_MASK AccessMask,
                       PSID Sid);

NTSTATUS
NTAPI
RtlAddAccessAllowedAceEx(PACL        Acl, ULONG       AceRevision, ULONG AceFlags,
                         ACCESS_MASK AccessMask, PSID Sid);

NTSTATUS
NTAPI
RtlAddAccessDeniedAce(PACL Acl, ULONG AceRevision, ACCESS_MASK AccessMask,
                      PSID Sid);

NTSTATUS
NTAPI
RtlAddAccessDeniedAceEx(PACL        Acl, ULONG       AceRevision, ULONG AceFlags,
                        ACCESS_MASK AccessMask, PSID Sid);

NTSTATUS
NTAPI
RtlAddAuditAccessAce(PACL Acl, ULONG   AceRevision, ACCESS_MASK AccessMask,
                     PSID Sid, BOOLEAN AuditSuccess, BOOLEAN    AuditFailure);

NTSTATUS
NTAPI
RtlAddAuditAccessAceEx(PACL        Acl, ULONG       AceRevision, ULONG AceFlags,
                       ACCESS_MASK AccessMask, PSID Sid, BOOLEAN       AuditSuccess,
                       BOOLEAN     AuditFailure);

NTSTATUS
NTAPI
RtlAddAccessAllowedObjectAce(_In_ _Out_ PACL Acl, _In_ ULONG AceRevision,
...
NTSTATUS
NTAPI
RtlAddAccessDeniedObjectAce(_In_ _Out_ PACL Acl, _In_ ULONG AceRevision,
                            _In_ ULONG AceFlags, _In_ ACCESS_MASK AccessMask,
                            _In_ GUID *ObjectTypeGuid OPTIONAL,
                            _In_ GUID *InheritedObjectTypeGuid OPTIONAL,
                            _In_ PSID Sid);
...
NTSTATUS
NTAPI
RtlAddAuditAccessObjectAce(_In_ _Out_ PACL Acl, _In_ ULONG AceRevision,
                           _In_ ULONG AceFlags, _In_ ACCESS_MASK AccessMask,
                           _In_ GUID *ObjectTypeGuid OPTIONAL,
                           _In_ GUID *InheritedObjectTypeGuid OPTIONAL,
                           _In_ PSID Sid, BOOLEAN AuditSuccess,
                           BOOLEAN                AuditFailure);
...
NTSTATUS
NTAPI
RtlAddCompoundAce(_In_ PACL Acl, _In_ ULONG AceRevision, _In_ UCHAR AceType,
                  _In_ ACCESS_MASK AccessMask, _In_ PSID ServerSid,
                  _In_ PSID ClientSid);
...
RtlCreateSecurityDescriptorRelative(
	PISECURITY_DESCRIPTOR_RELATIVE SecurityDescriptor, ULONG Revision);
...
NTSTATUS
NTAPI
RtlSetAttributesSecurityDescriptor(_In_ PSECURITY_DESCRIPTOR SecurityDescriptor,
...
BOOLEAN
NTAPI
RtlGetSecurityDescriptorRMControl(_In_ PSECURITY_DESCRIPTOR SecurityDescriptor,
                                  _Out_ PUCHAR RMControl);

VOID NTAPI RtlSetSecurityDescriptorRMControl(_In_ _Out_ PSECURITY_DESCRIPTOR
                                             SecurityDescriptor,
...
RtlSetGroupSecurityDescriptor(
	_In_ _Out_ PSECURITY_DESCRIPTOR SecurityDescriptor,
	_In_ PSID Group OPTIONAL, _In_ BOOLEAN GroupDefaulted OPTIONAL);
...
RtlSelfRelativeToAbsoluteSD(
	...
	_In_ _Out_ PULONG AbsoluteSecurityDescriptorSize, _Out_ OPTIONAL PACL Dacl,
	_In_ _Out_ PULONG DaclSize, _Out_ OPTIONAL PACL Sacl,
	_In_ _Out_ PULONG SaclSize, _Out_ OPTIONAL PSID Owner,
	_In_ _Out_ PULONG OwnerSize, _Out_ OPTIONAL PSID PrimaryGroup,
	_In_ _Out_ PULONG PrimaryGroupSize);

NTSTATUS
NTAPI
RtlSelfRelativeToAbsoluteSD2(_In_ _Out_ PSECURITY_DESCRIPTOR
                             pSelfRelativeSecurityDescriptor,
                             _In_ _Out_ PULONG pBufferSize);

NTSTATUS
NTAPI
RtlNewSecurityGrantedAccess(ACCESS_MASK    DesiredAccess,
                            PPRIVILEGE_SET Privileges, PULONG      Length,
...
NTSTATUS
NTAPI
RtlAdjustPrivilege(ULONG    Privilege, BOOLEAN Enable, BOOLEAN Client,
                   PBOOLEAN WasEnabled);

NTSTATUS
NTAPI
RtlAcquirePrivilege(PULONG Privilege, ULONG NumPriv, ULONG Flags,
                    PVOID* ReturnedState);

VOID NTAPI RtlReleasePrivilege(PVOID StatePointer);

VOID NTAPI RtlRunEncodeUnicodeString(PUCHAR Seed OPTIONAL,
...
NTSTATUS
NTAPI
RtlFindMessage(PVOID DllHandle, ULONG                    MessageTableId, ULONG MessageLanguageId,
               ULONG MessageId, PMESSAGE_RESOURCE_ENTRY* MessageEntry);

NTSTATUS
NTAPI
RtlFormatMessage(_In_ PWSTR MessageFormat, _In_ ULONG MaximumWidth,
...
NTSTATUS
NTAPI
RtlFormatMessageEx(_In_ PWSTR MessageFormat, _In_ ULONG MaximumWidth,
                   _In_ BOOLEAN IgnoreInserts, _In_ BOOLEAN ArgumentsAreAnsi,
                   _In_ BOOLEAN ArgumentsAreAnArray, _In_ va_list * Arguments,
                   _Out_ PWSTR Buffer, _In_ ULONG Length,
                   _Out_ OPTIONAL PULONG ReturnLength,
...
NTSTATUS
NTAPI
RtlInitializeRXact(_In_ HANDLE RootRegistryKey, _In_ BOOLEAN CommitIfNecessary,
                   _Out_ PRTL_RXACT_CONTEXT * RXactContext);

NTSTATUS
NTAPI
RtlStartRXact(_In_ PRTL_RXACT_CONTEXT RXactContext);

NTSTATUS
NTAPI
RtlAbortRXact(_In_ PRTL_RXACT_CONTEXT RXactContext);

NTSTATUS
NTAPI
RtlAddAttributeActionToRXact(_In_ PRTL_RXACT_CONTEXT RXactContext,
                             _In_ RTL_RXACT_OPERATION Operation,
                             _In_ PUNICODE_STRING SubKeyName,
                             _In_ HANDLE KeyHandle,
                             _In_ PUNICODE_STRING AttributeName,
                             _In_ ULONG NewValueType, _In_ PVOID NewValue,
                             _In_ ULONG NewValueLength);

NTSTATUS
NTAPI
RtlAddActionToRXact(_In_ PRTL_RXACT_CONTEXT RXactContext,
                    _In_ RTL_RXACT_OPERATION Operation,
                    _In_ PUNICODE_STRING SubKeyName, _In_ ULONG NewKeyValueType,
                    _In_ PVOID NewKeyValue OPTIONAL,
                    _In_ ULONG NewKeyValueLength);

NTSTATUS
NTAPI
RtlApplyRXact(_In_ PRTL_RXACT_CONTEXT RXactContext);

NTSTATUS
NTAPI
RtlApplyRXactNoFlush(_In_ PRTL_RXACT_CONTEXT RXactContext);
...
NTSTATUS
NTAPI
RtlCustomCPToUnicodeN(_In_ PCPTABLEINFO CustomCP, _Out_ PWCH UnicodeString,
                      _In_ ULONG MaxBytesInUnicodeString,
                      _Out_ OPTIONAL PULONG BytesInUnicodeString,
...
NTSTATUS
NTAPI
RtlUnicodeToCustomCPN(_In_ PCPTABLEINFO CustomCP, _Out_ PCH CustomCPString,
...
NTSTATUS
NTAPI
RtlUpcaseUnicodeToCustomCPN(_In_ PCPTABLEINFO CustomCP,
...
VOID NTAPI RtlInitCodePageTable(_In_ PUSHORT TableBase,
                                _Out_ PCPTABLEINFO CodePageTable);

VOID NTAPI RtlInitNlsTables(_In_ PUSHORT AnsiNlsBase, _In_ PUSHORT OemNlsBase,
                            _In_ PUSHORT LanguageNlsBase,
                            _Out_ PNLSTABLEINFO TableInfo);

VOID NTAPI RtlResetRtlTranslations(PNLSTABLEINFO TableInfo);

VOID NTAPI RtlGetDefaultCodePage(_Out_ PUSHORT AnsiCodePage,
                                 _Out_ PUSHORT OemCodePage);

VOID NTAPI RtlInitializeRangeList(_In_ _Out_ PRTL_RANGE_LIST RangeList);

VOID NTAPI RtlFreeRangeList(_In_ PRTL_RANGE_LIST RangeList);

NTSTATUS
NTAPI
RtlCopyRangeList(_Out_ PRTL_RANGE_LIST CopyRangeList,
                 _In_ PRTL_RANGE_LIST RangeList);

NTSTATUS
NTAPI
RtlAddRange(_In_ _Out_ PRTL_RANGE_LIST RangeList, _In_ ULONGLONG Start,
            _In_ ULONGLONG End, _In_ UCHAR Attributes, _In_ ULONG Flags,
            _In_ PVOID UserData, OPTIONAL _In_ PVOID Owner OPTIONAL);

NTSTATUS
NTAPI
RtlDeleteRange(_In_ _Out_ PRTL_RANGE_LIST RangeList, _In_ ULONGLONG Start,
               _In_ ULONGLONG End, _In_ PVOID Owner);

NTSTATUS
NTAPI
RtlDeleteOwnersRanges(_In_ _Out_ PRTL_RANGE_LIST RangeList, _In_ PVOID Owner);

NTSTATUS
NTAPI
RtlFindRange(_In_ PRTL_RANGE_LIST RangeList, _In_ ULONGLONG Minimum,
             _In_ ULONGLONG Maximum, _In_ ULONG Length, _In_ ULONG Alignment,
             _In_ ULONG Flags, _In_ UCHAR AttributeAvailableMask,
...
NTSTATUS
NTAPI
RtlIsRangeAvailable(_In_ PRTL_RANGE_LIST RangeList, _In_ ULONGLONG Start,
                    _In_ ULONGLONG End, _In_ ULONG Flags,
...
NTSTATUS
NTAPI
RtlGetFirstRange(_In_ PRTL_RANGE_LIST RangeList,
                 _Out_ PRTL_RANGE_LIST_ITERATOR Iterator,
                 _Out_ PRTL_RANGE * Range);

NTSTATUS
NTAPI
RtlGetLastRange(_In_ PRTL_RANGE_LIST RangeList,
                _Out_ PRTL_RANGE_LIST_ITERATOR Iterator,
                _Out_ PRTL_RANGE * Range);

NTSTATUS
NTAPI
RtlGetNextRange(_In_ _Out_ PRTL_RANGE_LIST_ITERATOR Iterator,
                _Out_ PRTL_RANGE * Range, _In_ BOOLEAN MoveForwards);

NTSTATUS
NTAPI
RtlMergeRangeLists(_Out_ PRTL_RANGE_LIST MergedRangeList,
                   _In_ PRTL_RANGE_LIST RangeList1,
                   _In_ PRTL_RANGE_LIST RangeList2, _In_ ULONG Flags);

NTSTATUS
NTAPI
RtlInvertRangeList(_Out_ PRTL_RANGE_LIST InvertedRangeList,
                   _In_ PRTL_RANGE_LIST RangeList);

NTSTATUS
NTAPI
RtlVolumeDeviceToDosName(_In_ PVOID VolumeDeviceObject,
                         _Out_ PUNICODE_STRING DosName);

NTSTATUS
NTAPI
RtlCreateSystemVolumeInformationFolder(_In_ PUNICODE_STRING VolumeRootPath);
#if defined(_WINNT_) && (_MSC_VER < 1300)
...
NTSTATUS RtlVerifyVersionInfo(_In_ PRTL_OSVERSIONINFOEXW VersionInfo,
                              _In_ ULONG TypeMask, _In_ ULONGLONG ConditionMask);

BOOLEAN RtlFlushSecureMemoryCache(PVOID lpAddr, SIZE_T size);
...
NtSetSystemEnvironmentValueEx(_In_ PUNICODE_STRING VariableName,
                              _In_ LPGUID VendorGuid, _In_ OPTIONAL PVOID Value,
...
NTSTATUS
NTAPI
NtAddBootEntry(_In_ PBOOT_ENTRY BootEntry, _Out_ OPTIONAL PULONG Id);
...
NTSTATUS
NTAPI
NtEnumerateBootEntries(_Out_ OPTIONAL PVOID Buffer,
                       _In_ _Out_ PULONG BufferLength);

NTSTATUS
NTAPI
NtQueryBootEntryOrder(_Out_ OPTIONAL PULONG Ids, _In_ _Out_ PULONG Count);

NTSTATUS
NTAPI
NtSetBootEntryOrder(_In_ PULONG Ids, _In_ ULONG Count);

NTSTATUS
NTAPI
NtQueryBootOptions(_Out_ OPTIONAL PBOOT_OPTIONS BootOptions,
...
NTSTATUS
NTAPI
NtAddDriverEntry(_In_ PEFI_DRIVER_ENTRY DriverEntry, _Out_ OPTIONAL PULONG Id);
...
NTSTATUS
NTAPI
NtEnumerateDriverEntries(_Out_ PVOID Buffer, _In_ _Out_ PULONG BufferLength);

NTSTATUS
NTAPI
NtQueryDriverEntryOrder(_Out_ PULONG Ids, _In_ _Out_ PULONG Count);

NTSTATUS
NTAPI
NtSetDriverEntryOrder(_In_ PULONG Ids, _In_ ULONG Count);
...
NTSTATUS
NTAPI NtCreateThreadEx(PHANDLE hThread, ACCESS_MASK      DesiredAccess,
...
NTSTATUS
NTAPI
NtPulseEvent(_In_ HANDLE EventHandle, _Out_ OPTIONAL PLONG PreviousState);

NTSTATUS
NTAPI
NtQueryEvent(_In_ HANDLE EventHandle,
             _In_ EVENT_INFORMATION_CLASS EventInformationClass,
             _Out_ PVOID EventInformation, _In_ ULONG EventInformationLength,
             _Out_ OPTIONAL PULONG ReturnLength);

NTSTATUS
NTAPI
NtResetEvent(_In_ HANDLE EventHandle, _Out_ OPTIONAL PLONG PreviousState);

NTSTATUS
NTAPI
NtSetEvent(_In_ HANDLE EventHandle, _Out_ OPTIONAL PLONG PreviousState);

NTSTATUS
NTAPI
NtSetEventBoostPriority(_In_ HANDLE EventHandle);

NTSTATUS
NTAPI
NtCreateEventPair(_Out_ PHANDLE EventPairHandle, _In_ ACCESS_MASK DesiredAccess,
                  _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
NtOpenEventPair(_Out_ PHANDLE EventPairHandle, _In_ ACCESS_MASK DesiredAccess,
                _In_ POBJECT_ATTRIBUTES ObjectAttributes);
...
NtQueryMutant(_In_ HANDLE MutantHandle,
              ...
              _Out_ PVOID MutantInformation, _In_ ULONG MutantInformationLength,
              _Out_ OPTIONAL PULONG ReturnLength);

NTSTATUS
NTAPI
NtReleaseMutant(_In_ HANDLE MutantHandle, _Out_ OPTIONAL PLONG PreviousCount);
...
NTSTATUS
NTAPI
NtCancelTimer(_In_ HANDLE TimerHandle, _Out_ OPTIONAL PBOOLEAN CurrentState);
...
NtSetTimer(_In_ HANDLE TimerHandle, _In_ PLARGE_INTEGER DueTime,
           ...
           _In_ OPTIONAL PVOID TimerContext, _In_ BOOLEAN ResumeTimer,
           _In_ OPTIONAL LONG Period, _Out_ OPTIONAL PBOOLEAN PreviousState);
...
NTSTATUS
NTAPI
NtSetSystemTime(_In_ OPTIONAL PLARGE_INTEGER SystemTime,
...
NTSTATUS
NTAPI
NtCreateProfile(_Out_ PHANDLE ProfileHandle, _In_ HANDLE Process OPTIONAL,
                _In_ PVOID ProfileBase, _In_ SIZE_T ProfileSize,
                _In_ ULONG BucketSize, _In_ PULONG Buffer,
                _In_ ULONG BufferSize, _In_ KPROFILE_SOURCE ProfileSource,
...
NTSTATUS
NTAPI
NtQueryIntervalProfile(_In_ KPROFILE_SOURCE ProfileSource,
...
NtRaiseHardError(_In_ NTSTATUS ErrorStatus, _In_ ULONG NumberOfParameters,
                 ...
                 _In_ ULONG ValidResponseOptions, _Out_ PULONG Response);

NTSTATUS
NTAPI
NtQueryDefaultLocale(_In_ BOOLEAN UserProfile, _Out_ PLCID DefaultLocaleId);

NTSTATUS
NTAPI
NtSetDefaultLocale(_In_ BOOLEAN UserProfile, _In_ LCID DefaultLocaleId);

NTSTATUS
NTAPI
NtQueryInstallUILanguage(_Out_ LANGID * InstallUILanguageId);

NTSTATUS
NTAPI
NtQueryDefaultUILanguage(_Out_ LANGID * DefaultUILanguageId);

NTSTATUS
NTAPI
NtSetDefaultUILanguage(_In_ LANGID DefaultUILanguageId);

NTSTATUS
NTAPI
NtSetDefaultHardErrorPort(_In_ HANDLE DefaultHardErrorPort);

NTSTATUS
NTAPI
NtShutdownSystem(_In_ SHUTDOWN_ACTION Action);
...
NTSTATUS
NTAPI
NtAddAtom(_In_ OPTIONAL PWSTR AtomName, _In_ ULONG Length,
          _Out_ OPTIONAL PRTL_ATOM Atom);

NTSTATUS
NTAPI
NtFindAtom(_In_ OPTIONAL PWSTR AtomName, _In_ ULONG Length,
           _Out_ OPTIONAL PRTL_ATOM Atom);

NTSTATUS
NTAPI
NtDeleteAtom(_In_ RTL_ATOM Atom);

NTSTATUS
NTAPI
NtQueryInformationAtom(_In_ RTL_ATOM Atom,
...
NTSTATUS
NTAPI
NtNotifyChangeDirectoryFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
...
NtQueryEaFile(_In_ HANDLE FileHandle, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
              ...
              _In_ ULONG EaListLength, _In_ OPTIONAL PULONG EaIndex OPTIONAL,
...
NtCreateFile(_Out_ PHANDLE FileHandle, _In_ ACCESS_MASK DesiredAccess,
             ...
             _In_ OPTIONAL PVOID EaBuffer, _In_ ULONG EaLength);

NTSTATUS
NTAPI
NtDeviceIoControlFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                      _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                      _In_ OPTIONAL PVOID ApcContext,
                      _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                      _In_ ULONG IoControlCode, _In_ OPTIONAL PVOID InputBuffer,
                      _In_ ULONG InputBufferLength,
                      _Out_ OPTIONAL PVOID OutputBuffer,
                      _In_ ULONG OutputBufferLength);
...
NTSTATUS
NTAPI
NtFsControlFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                _In_ OPTIONAL PVOID ApcContext,
                _Out_ PIO_STATUS_BLOCK IoStatusBlock, _In_ ULONG FsControlCode,
                _In_ OPTIONAL PVOID InputBuffer, _In_ ULONG InputBufferLength,
                _Out_ OPTIONAL PVOID OutputBuffer,
                _In_ ULONG OutputBufferLength);

NTSTATUS
NTAPI
NtLockFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
           _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
           _In_ OPTIONAL PVOID ApcContext, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
           _In_ PLARGE_INTEGER ByteOffset, _In_ PLARGE_INTEGER Length,
           _In_ ULONG Key, _In_ BOOLEAN FailImmediately,
...
NTSTATUS
NTAPI
NtQueryDirectoryFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                     _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                     _In_ OPTIONAL PVOID ApcContext,
                     _Out_ PIO_STATUS_BLOCK IoStatusBlock,
...
NtQueryQuotaInformationFile(
	...
	_In_ OPTIONAL PVOID SidList, _In_ ULONG SidListLength,
	_In_ OPTIONAL PSID StartSid, _In_ BOOLEAN RestartScan);
...
NTSTATUS
NTAPI
NtReadFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
           _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
           _In_ OPTIONAL PVOID ApcContext, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
           _Out_ PVOID Buffer, _In_ ULONG Length,
           _In_ OPTIONAL PLARGE_INTEGER ByteOffset, _In_ OPTIONAL PULONG Key);
...
NTSTATUS
NTAPI
NtWriteFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
            _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
            _In_ OPTIONAL PVOID ApcContext,
            _Out_ PIO_STATUS_BLOCK IoStatusBlock, _In_ PVOID Buffer,
            _In_ ULONG Length, _In_ OPTIONAL PLARGE_INTEGER ByteOffset,
...
NTSTATUS
NTAPI
NtReadFileScatter(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                  _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                  _In_ OPTIONAL PVOID ApcContext,
                  _Out_ PIO_STATUS_BLOCK IoStatusBlock,
...
NTSTATUS
NTAPI
NtWriteFileGather(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                  _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                  _In_ OPTIONAL PVOID ApcContext,
                  _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                  _In_ PFILE_SEGMENT_ELEMENT SegmentArray, _In_ ULONG Length,
...
NtQueryIoCompletion(_In_ HANDLE IoCompletionHandle,
                    _In_ IO_COMPLETION_INFORMATION_CLASS
...
NTSTATUS
NTAPI
NtSetIoCompletion(_In_ HANDLE IoCompletionHandle, _In_ PVOID KeyContext,
                  _In_ OPTIONAL PVOID ApcContext, _In_ NTSTATUS IoStatus,
...
NTSTATUS
NTAPI
NtCallbackReturn(_In_ PVOID OutputBuffer OPTIONAL, _In_ ULONG OutputLength,
...
NTSTATUS
NTAPI
NtCreatePort(_Out_ PHANDLE PortHandle, _In_ POBJECT_ATTRIBUTES ObjectAttributes,
             _In_ ULONG MaxConnectionInfoLength, _In_ ULONG MaxMessageLength,
             _In_ OPTIONAL ULONG MaxPoolUsage);

NTSTATUS
NTAPI
NtCreateWaitablePort(_Out_ PHANDLE PortHandle,
                     _In_ POBJECT_ATTRIBUTES ObjectAttributes,
...
NTSTATUS
NTAPI
NtConnectPort(_Out_ PHANDLE PortHandle, _In_ PUNICODE_STRING PortName,
...
NTSTATUS
NTAPI
NtSecureConnectPort(_Out_ PHANDLE PortHandle, _In_ PUNICODE_STRING PortName,
                    _In_ PSECURITY_QUALITY_OF_SERVICE SecurityQos,
                    _In_ _Out_ OPTIONAL PPORT_VIEW ClientView,
...
NTSTATUS
NTAPI
NtAcceptConnectPort(_Out_ PHANDLE PortHandle, _In_ OPTIONAL PVOID PortContext,
...
NTSTATUS
NTAPI
NtRequestWaitReplyPort(_In_ HANDLE PortHandle,
...
NTSTATUS
NTAPI
NtReplyWaitReplyPort(_In_ HANDLE PortHandle,
                     _In_ _Out_ PPORT_MESSAGE ReplyMessage);

NTSTATUS
NTAPI
NtReplyWaitReceivePort(_In_ HANDLE PortHandle,
...
NTSTATUS
NTAPI
NtReplyWaitReceivePortEx(_In_ HANDLE PortHandle,
                         _Out_ OPTIONAL PVOID * PortContext,
                         _In_ OPTIONAL PPORT_MESSAGE ReplyMessage,
                         _Out_ PPORT_MESSAGE ReceiveMessage,
                         _In_ OPTIONAL PLARGE_INTEGER Timeout);
...
NTSTATUS
NTAPI
NtImpersonateClientOfPort(_In_ HANDLE PortHandle, _In_ PPORT_MESSAGE Message);

NTSTATUS
NTAPI
NtReadRequestData(_In_ HANDLE PortHandle, _In_ PPORT_MESSAGE Message,
                  _In_ ULONG DataEntryIndex, _Out_ PVOID Buffer,
                  _In_ SIZE_T BufferSize,
                  _Out_ OPTIONAL PSIZE_T NumberOfBytesRead);

NTSTATUS
NTAPI
NtWriteRequestData(_In_ HANDLE PortHandle, _In_ PPORT_MESSAGE Message,
                   _In_ ULONG DataEntryIndex, _In_ PVOID Buffer,
                   _In_ SIZE_T BufferSize,
                   _Out_ OPTIONAL PSIZE_T NumberOfBytesWritten);

NTSTATUS
NTAPI
NtQueryInformationPort(_In_ HANDLE PortHandle,
                       _In_ PORT_INFORMATION_CLASS PortInformationClass,
                       _Out_ PVOID PortInformation, _In_ ULONG Length,
                       _Out_ OPTIONAL PULONG ReturnLength);
...
NtMapViewOfSection(_In_ HANDLE SectionHandle, _In_ HANDLE ProcessHandle,
                   _In_ _Out_ PVOID * BaseAddress, _In_ ULONG_PTR ZeroBits,
...
NTSTATUS
NTAPI
NtFreeVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                    _In_ _Out_ PSIZE_T RegionSize, _In_ ULONG FreeType);

NTSTATUS
NTAPI
NtReadVirtualMemory(_In_ HANDLE ProcessHandle, _In_ OPTIONAL PVOID BaseAddress,
                    _Out_ PVOID Buffer, _In_ SIZE_T BufferSize,
                    _Out_ OPTIONAL PSIZE_T NumberOfBytesRead);

NTSTATUS
NTAPI
NtWriteVirtualMemory(_In_ HANDLE ProcessHandle, _In_ OPTIONAL PVOID BaseAddress,
...
NTSTATUS NtSetInformationVirtualMemory(
	...
	_In_ ULONG_PTR NumberOfEntries, _In_ PMEMORY_RANGE_ENTRY VirtualAddresses,
	_In_ PVOID VmInformation, _In_ ULONG VmInformationLength);

NTSTATUS
NTAPI
NtFlushVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                     _In_ _Out_ PSIZE_T RegionSize,
                     _Out_ PIO_STATUS_BLOCK IoStatus);

NTSTATUS
NTAPI
NtLockVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                    _In_ _Out_ PSIZE_T RegionSize, _In_ ULONG MapType);

NTSTATUS
NTAPI
NtUnlockVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                      _In_ _Out_ PSIZE_T RegionSize, _In_ ULONG MapType);

NTSTATUS
NTAPI
NtProtectVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                       _In_ _Out_ PSIZE_T RegionSize, _In_ ULONG NewProtect,
...
NTSTATUS
NTAPI
NtGetWriteWatch(_In_ HANDLE ProcessHandle, _In_ ULONG Flags,
                _In_ PVOID BaseAddress, _In_ SIZE_T RegionSize,
...
NTSTATUS
NTAPI
NtResetWriteWatch(_In_ HANDLE ProcessHandle, _In_ PVOID BaseAddress,
                  _In_ SIZE_T RegionSize);

NTSTATUS
NTAPI
NtCreatePagingFile(_In_ PUNICODE_STRING PageFileName,
                   _In_ PLARGE_INTEGER MinimumSize,
                   _In_ PLARGE_INTEGER MaximumSize, _In_ ULONG Priority);

NTSTATUS
NTAPI
NtFlushInstructionCache(_In_ HANDLE ProcessHandle,
                        _In_ OPTIONAL PVOID BaseAddress, _In_ SIZE_T Length);
...
NtQueryObject(_In_ HANDLE Handle,
              ...
              _Out_ PVOID ObjectInformation, _In_ ULONG ObjectInformationLength,
...
NtDuplicateObject(_In_ HANDLE SourceProcessHandle, _In_ HANDLE SourceHandle,
                  ...
                  _Out_ PHANDLE TargetHandle, _In_ ACCESS_MASK DesiredAccess,
                  _In_ ULONG HandleAttributes, _In_ ULONG Options);
...
NTSTATUS
NTAPI
NtWaitForMultipleObjects(_In_ ULONG Count, _In_ HANDLE Handles[],
                         _In_ WAIT_TYPE WaitType, _In_ BOOLEAN Alertable,
                         _In_ OPTIONAL PLARGE_INTEGER Timeout);

NTSTATUS
NTAPI
NtWaitForMultipleObjects32(_In_ ULONG Count, _In_ LONG Handles[],
                           _In_ WAIT_TYPE WaitType, _In_ BOOLEAN Alertable,
                           _In_ OPTIONAL PLARGE_INTEGER Timeout);
...
NtQueryDirectoryObject(_In_ HANDLE DirectoryHandle, _Out_ PVOID Buffer,
                       ...
                       _In_ BOOLEAN RestartScan, _In_ _Out_ PULONG Context,
                       _Out_ PULONG ReturnLength);
...
NTSTATUS
NTAPI
NtGetPlugPlayEvent(_In_ HANDLE EventHandle, _In_ OPTIONAL PVOID Context,
                   _Out_ PPLUGPLAY_EVENT_BLOCK EventBlock,
                   _In_ ULONG EventBufferSize);

NTSTATUS
NTAPI
NtPlugPlayControl(_In_ PLUGPLAY_CONTROL_CLASS PnPControlClass,
                  _In_ _Out_ PVOID PnPControlData,
                  _In_ ULONG PnPControlDataLength);

NTSTATUS
NTAPI
NtPowerInformation(_In_ POWER_INFORMATION_LEVEL InformationLevel,
                   _In_ OPTIONAL PVOID InputBuffer,
                   _In_ ULONG InputBufferLength,
                   _Out_ OPTIONAL PVOID OutputBuffer,
                   _In_ ULONG OutputBufferLength);
...
NTSTATUS
NTAPI
NtSetThreadExecutionState(_In_ EXECUTION_STATE esFlags,
                          _Out_ EXECUTION_STATE * PreviousFlags);

NTSTATUS
NTAPI
NtRequestWakeupLatency(_In_ LATENCY_TIME latency);

NTSTATUS
NTAPI
NtCancelDeviceWakeupRequest(_In_ HANDLE Device);

NTSTATUS
NTAPI
NtRequestDeviceWakeup(_In_ HANDLE Device);
...
NTSTATUS
NTAPI
NtTerminateProcess(_In_ OPTIONAL HANDLE ProcessHandle,
...
NTSTATUS
NTAPI
NtTerminateThread(_In_ OPTIONAL HANDLE ThreadHandle, _In_ NTSTATUS ExitStatus);
...
NTSTATUS
NTAPI
NtResumeThread(_In_ HANDLE ThreadHandle,
               _Out_ OPTIONAL PULONG PreviousSuspendCount);
...
NTSTATUS
NTAPI
NtGetContextThread(_In_ HANDLE ThreadHandle, _In_ _Out_ PCONTEXT ThreadContext);
...
NTSTATUS
NTAPI
NtIsProcessInJob(_In_ HANDLE ProcessHandle, _In_ OPTIONAL HANDLE JobHandle);

NTSTATUS
NTAPI
NtCreateJobSet(_In_ ULONG NumJob, _In_ PJOB_SET_ARRAY UserJobSet,
               _In_ ULONG Flags);

NTSTATUS
NTAPI
NtQueryInformationJobObject(_In_ OPTIONAL HANDLE JobHandle,
...
NtCreateKey(_Out_ PHANDLE KeyHandle, _In_ ACCESS_MASK DesiredAccess,
            _In_ POBJECT_ATTRIBUTES ObjectAttributes, ULONG TitleIndex,
            _In_ OPTIONAL PUNICODE_STRING Class, _In_ ULONG CreateOptions,
...
NtEnumerateKey(_In_ HANDLE KeyHandle, _In_ ULONG Index,
               ...
               _Out_ OPTIONAL PVOID KeyInformation, _In_ ULONG Length,
...
NtEnumerateValueKey(_In_ HANDLE KeyHandle, _In_ ULONG Index,
                    ...
                    _Out_ OPTIONAL PVOID KeyValueInformation, _In_ ULONG Length,
                    _Out_ PULONG ResultLength);
...
NTSTATUS
NTAPI
NtNotifyChangeKey(_In_ HANDLE KeyHandle, _In_ OPTIONAL HANDLE Event,
                  _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                  _In_ OPTIONAL PVOID ApcContext,
                  _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                  _In_ ULONG CompletionFilter, _In_ BOOLEAN WatchTree,
                  _Out_ OPTIONAL PVOID Buffer, _In_ ULONG BufferSize,
                  _In_ BOOLEAN Asynchronous);

NTSTATUS
NTAPI
NtNotifyChangeMultipleKeys(
	_In_ HANDLE MasterKeyHandle, _In_ OPTIONAL ULONG Count,
	_In_ OPTIONAL OBJECT_ATTRIBUTES SlaveObjects[], _In_ OPTIONAL HANDLE Event,
	_In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine, _In_ OPTIONAL PVOID ApcContext,
	_Out_ PIO_STATUS_BLOCK IoStatusBlock, _In_ ULONG CompletionFilter,
	_In_ BOOLEAN WatchTree, _Out_ OPTIONAL PVOID Buffer, _In_ ULONG BufferSize,
	_In_ BOOLEAN Asynchronous);
...
NTSTATUS
NTAPI
NtCompactKeys(_In_ ULONG Count, _In_ HANDLE KeyArray[]);
...
NtSetValueKey(_In_ HANDLE KeyHandle, _In_ PUNICODE_STRING ValueName,
              _In_ OPTIONAL ULONG TitleIndex, _In_ ULONG Type,
              _In_ OPTIONAL PVOID Data, _In_ ULONG DataSize);
...
NTSTATUS
NTAPI
NtUnloadKeyEx(_In_ POBJECT_ATTRIBUTES TargetKey, _In_ OPTIONAL HANDLE Event);
...
NTSTATUS
NTAPI
NtLockProductActivationKeys(_In_ _Out_ OPTIONAL ULONG * pPrivateVer,
...
NtAccessCheckByType(
	_In_ PSECURITY_DESCRIPTOR SecurityDescriptor,
	_In_ OPTIONAL PSID PrincipalSelfSid, _In_ HANDLE ClientToken,
	...
	_Out_ PPRIVILEGE_SET PrivilegeSet, _In_ _Out_ PULONG PrivilegeSetLength,
	_Out_ PACCESS_MASK GrantedAccess, _Out_ PNTSTATUS AccessStatus);
...
NtAccessCheckAndAuditAlarm(
	_In_ PUNICODE_STRING SubsystemName, _In_ OPTIONAL PVOID HandleId,
	_In_ PUNICODE_STRING ObjectTypeName, _In_ PUNICODE_STRING ObjectName,
	_In_ PSECURITY_DESCRIPTOR SecurityDescriptor,
	_In_ ACCESS_MASK DesiredAccess, _In_ PGENERIC_MAPPING GenericMapping,
	_In_ BOOLEAN ObjectCreation, _Out_ PACCESS_MASK GrantedAccess,
	_Out_ PNTSTATUS AccessStatus, _Out_ PBOOLEAN GenerateOnClose);
...
NtAccessCheckByTypeAndAuditAlarm(
	...
	_In_ PUNICODE_STRING ObjectTypeName, _In_ PUNICODE_STRING ObjectName,
	_In_ PSECURITY_DESCRIPTOR SecurityDescriptor,
	_In_ OPTIONAL PSID PrincipalSelfSid, _In_ ACCESS_MASK DesiredAccess,
	_In_ AUDIT_EVENT_TYPE AuditType, _In_ ULONG Flags,
	_In_ POBJECT_TYPE_LIST ObjectTypeList, _In_ ULONG ObjectTypeListLength,
	_In_ PGENERIC_MAPPING GenericMapping, _In_ BOOLEAN ObjectCreation,
	_Out_ PACCESS_MASK GrantedAccess, _Out_ PNTSTATUS AccessStatus,
...
NtAccessCheckByTypeResultListAndAuditAlarmByHandle(
	_In_ PUNICODE_STRING SubsystemName, _In_ OPTIONAL PVOID HandleId,
	_In_ HANDLE ClientToken, _In_ PUNICODE_STRING ObjectTypeName,
...
NtOpenObjectAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                       ...
                       _In_ BOOLEAN ObjectCreation, _In_ BOOLEAN AccessGranted,
                       _Out_ PBOOLEAN GenerateOnClose);

NTSTATUS
NTAPI
NtPrivilegeObjectAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                            _In_ OPTIONAL PVOID HandleId,
...
NTSTATUS
NTAPI
NtCloseObjectAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                        _In_ OPTIONAL PVOID HandleId,
                        _In_ BOOLEAN GenerateOnClose);

NTSTATUS
NTAPI
NtDeleteObjectAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                         _In_ OPTIONAL PVOID HandleId,
                         _In_ BOOLEAN GenerateOnClose);

NTSTATUS
NTAPI
NtPrivilegedServiceAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
...
NTSTATUS
NTAPI
ZwDelayExecution(_In_ BOOLEAN Alertable, _In_ PLARGE_INTEGER DelayInterval);

NTSTATUS
NTAPI
ZwQuerySystemEnvironmentValue(_In_ PUNICODE_STRING VariableName,
                              _Out_ PWSTR VariableValue,
                              _In_ USHORT ValueLength,
                              _Out_ OPTIONAL PUSHORT ReturnLength);
...
NTSTATUS
NTAPI
ZwSetSystemEnvironmentValue(_In_ PUNICODE_STRING VariableName,
                            _In_ PUNICODE_STRING VariableValue);

NTSTATUS
NTAPI
ZwQuerySystemEnvironmentValueEx(_In_ PUNICODE_STRING VariableName,
                                _In_ LPGUID VendorGuid,
                                _Out_ OPTIONAL PVOID Value,
                                _In_ _Out_ PULONG ValueLength,
                                _Out_ OPTIONAL PULONG Attributes);
...
NTSTATUS
NTAPI
ZwSetSystemEnvironmentValueEx(_In_ PUNICODE_STRING VariableName,
                              _In_ LPGUID VendorGuid, _In_ OPTIONAL PVOID Value,
                              _In_ ULONG ValueLength, _In_ ULONG Attributes);

NTSTATUS
NTAPI
ZwEnumerateSystemEnvironmentValuesEx(_In_ ULONG InformationClass,
                                     _Out_ PVOID Buffer,
                                     _In_ _Out_ PULONG BufferLength);

NTSTATUS
NTAPI
ZwAddBootEntry(_In_ PBOOT_ENTRY BootEntry, _Out_ OPTIONAL PULONG Id);

NTSTATUS
NTAPI
ZwDeleteBootEntry(_In_ ULONG Id);

NTSTATUS
NTAPI
ZwModifyBootEntry(_In_ PBOOT_ENTRY BootEntry);

NTSTATUS
NTAPI
ZwEnumerateBootEntries(_Out_ OPTIONAL PVOID Buffer,
                       _In_ _Out_ PULONG BufferLength);

NTSTATUS
NTAPI
ZwQueryBootEntryOrder(_Out_ OPTIONAL PULONG Ids, _In_ _Out_ PULONG Count);

NTSTATUS
NTAPI
ZwSetBootEntryOrder(_In_ PULONG Ids, _In_ ULONG Count);

NTSTATUS
NTAPI
ZwQueryBootOptions(_Out_ OPTIONAL PBOOT_OPTIONS BootOptions,
                   _In_ _Out_ PULONG BootOptionsLength);

NTSTATUS
NTAPI
ZwSetBootOptions(_In_ PBOOT_OPTIONS BootOptions, _In_ ULONG FieldsToChange);

NTSTATUS
NTAPI
ZwTranslateFilePath(_In_ PFILE_PATH InputFilePath, _In_ ULONG OutputType,
                    _Out_ OPTIONAL PFILE_PATH OutputFilePath,
                    _In_ _Out_ OPTIONAL PULONG OutputFilePathLength);

NTSTATUS
NTAPI
ZwAddDriverEntry(_In_ PEFI_DRIVER_ENTRY DriverEntry, _Out_ OPTIONAL PULONG Id);

NTSTATUS
NTAPI
ZwDeleteDriverEntry(_In_ ULONG Id);

NTSTATUS
NTAPI
ZwModifyDriverEntry(_In_ PEFI_DRIVER_ENTRY DriverEntry);

NTSTATUS
NTAPI
ZwEnumerateDriverEntries(_Out_ PVOID Buffer, _In_ _Out_ PULONG BufferLength);

NTSTATUS
NTAPI
ZwQueryDriverEntryOrder(_Out_ PULONG Ids, _In_ _Out_ PULONG Count);

NTSTATUS
NTAPI
ZwSetDriverEntryOrder(_In_ PULONG Ids, _In_ ULONG Count);

NTSTATUS
NTAPI
ZwClearEvent(_In_ HANDLE EventHandle);
...
NTSTATUS
NTAPI
ZwOpenEvent(_Out_ PHANDLE EventHandle, _In_ ACCESS_MASK DesiredAccess,
            _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwPulseEvent(_In_ HANDLE EventHandle, _Out_ OPTIONAL PLONG PreviousState);

NTSTATUS
NTAPI
ZwQueryEvent(_In_ HANDLE EventHandle,
             _In_ EVENT_INFORMATION_CLASS EventInformationClass,
             _Out_ PVOID EventInformation, _In_ ULONG EventInformationLength,
             _Out_ OPTIONAL PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwResetEvent(_In_ HANDLE EventHandle, _Out_ OPTIONAL PLONG PreviousState);

NTSTATUS
NTAPI
ZwSetEvent(_In_ HANDLE EventHandle, _Out_ OPTIONAL PLONG PreviousState);

NTSTATUS
NTAPI
ZwSetEventBoostPriority(_In_ HANDLE EventHandle);

NTSTATUS
NTAPI
ZwCreateEventPair(_Out_ PHANDLE EventPairHandle, _In_ ACCESS_MASK DesiredAccess,
                  _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwOpenEventPair(_Out_ PHANDLE EventPairHandle, _In_ ACCESS_MASK DesiredAccess,
                _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwWaitLowEventPair(_In_ HANDLE EventPairHandle);

NTSTATUS
NTAPI
ZwWaitHighEventPair(_In_ HANDLE EventPairHandle);

NTSTATUS
NTAPI
ZwSetLowWaitHighEventPair(_In_ HANDLE EventPairHandle);

NTSTATUS
NTAPI
ZwSetHighWaitLowEventPair(_In_ HANDLE EventPairHandle);

NTSTATUS
NTAPI
ZwSetLowEventPair(_In_ HANDLE EventPairHandle);

NTSTATUS
NTAPI
ZwSetHighEventPair(_In_ HANDLE EventPairHandle);

NTSTATUS
NTAPI
ZwCreateMutant(_Out_ PHANDLE MutantHandle, _In_ ACCESS_MASK DesiredAccess,
               _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
               _In_ BOOLEAN InitialOwner);

NTSTATUS
NTAPI
ZwOpenMutant(_Out_ PHANDLE MutantHandle, _In_ ACCESS_MASK DesiredAccess,
             _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwQueryMutant(_In_ HANDLE MutantHandle,
              _In_ MUTANT_INFORMATION_CLASS MutantInformationClass,
              _Out_ PVOID MutantInformation, _In_ ULONG MutantInformationLength,
              _Out_ OPTIONAL PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwReleaseMutant(_In_ HANDLE MutantHandle, _Out_ OPTIONAL PLONG PreviousCount);

NTSTATUS
NTAPI
ZwCreateSemaphore(_Out_ PHANDLE SemaphoreHandle, _In_ ACCESS_MASK DesiredAccess,
                  _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
                  _In_ LONG InitialCount, _In_ LONG MaximumCount);

NTSTATUS
NTAPI
ZwOpenSemaphore(_Out_ PHANDLE SemaphoreHandle, _In_ ACCESS_MASK DesiredAccess,
                _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwQuerySemaphore(_In_ HANDLE SemaphoreHandle,
                 _In_ SEMAPHORE_INFORMATION_CLASS SemaphoreInformationClass,
                 _Out_ PVOID SemaphoreInformation,
                 _In_ ULONG SemaphoreInformationLength,
                 _Out_ OPTIONAL PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwReleaseSemaphore(_In_ HANDLE SemaphoreHandle, _In_ LONG ReleaseCount,
                   _Out_ OPTIONAL PLONG PreviousCount);

NTSTATUS
NTAPI
ZwCreateTimer(_Out_ PHANDLE TimerHandle, _In_ ACCESS_MASK DesiredAccess,
              _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
              _In_ TIMER_TYPE TimerType);

NTSTATUS
NTAPI
ZwOpenTimer(_Out_ PHANDLE TimerHandle, _In_ ACCESS_MASK DesiredAccess,
            _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwCancelTimer(_In_ HANDLE TimerHandle, _Out_ OPTIONAL PBOOLEAN CurrentState);

NTSTATUS
NTAPI
ZwQueryTimer(_In_ HANDLE TimerHandle,
             _In_ TIMER_INFORMATION_CLASS TimerInformationClass,
             _Out_ PVOID TimerInformation, _In_ ULONG TimerInformationLength,
             _Out_ OPTIONAL PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwSetTimer(_In_ HANDLE TimerHandle, _In_ PLARGE_INTEGER DueTime,
           _In_ OPTIONAL PTIMER_APC_ROUTINE TimerApcRoutine,
           _In_ OPTIONAL PVOID TimerContext, _In_ BOOLEAN ResumeTimer,
           _In_ OPTIONAL LONG Period, _Out_ OPTIONAL PBOOLEAN PreviousState);
...
NTSTATUS
NTAPI
ZwQuerySystemTime(_Out_ PLARGE_INTEGER SystemTime);

NTSTATUS
NTAPI
ZwSetSystemTime(_In_ OPTIONAL PLARGE_INTEGER SystemTime,
                _Out_ OPTIONAL PLARGE_INTEGER PreviousTime);

NTSTATUS
NTAPI
ZwQueryTimerResolution(_Out_ PULONG MaximumTime, _Out_ PULONG MinimumTime,
                       _Out_ PULONG CurrentTime);

NTSTATUS
NTAPI
ZwSetTimerResolution(_In_ ULONG DesiredTime, _In_ BOOLEAN SetResolution,
                     _Out_ PULONG ActualTime);

NTSTATUS
NTAPI
ZwAllocateLocallyUniqueId(_Out_ PLUID Luid);

NTSTATUS
NTAPI
ZwSetUuidSeed(_In_ PCHAR Seed);

NTSTATUS
NTAPI
ZwAllocateUuids(_Out_ PULARGE_INTEGER Time, _Out_ PULONG Range,
                _Out_ PULONG Sequence, _Out_ PCHAR Seed);

NTSTATUS
NTAPI
ZwCreateProfile(_Out_ PHANDLE ProfileHandle, _In_ HANDLE Process OPTIONAL,
                _In_ PVOID ProfileBase, _In_ SIZE_T ProfileSize,
                _In_ ULONG BucketSize, _In_ PULONG Buffer,
                _In_ ULONG BufferSize, _In_ KPROFILE_SOURCE ProfileSource,
                _In_ KAFFINITY Affinity);
...
NTSTATUS
NTAPI
ZwStartProfile(_In_ HANDLE ProfileHandle);

NTSTATUS
NTAPI
ZwStopProfile(_In_ HANDLE ProfileHandle);

NTSTATUS
NTAPI
ZwSetIntervalProfile(_In_ ULONG Interval, _In_ KPROFILE_SOURCE Source);

NTSTATUS
NTAPI
ZwQueryIntervalProfile(_In_ KPROFILE_SOURCE ProfileSource,
                       _Out_ PULONG Interval);

NTSTATUS
NTAPI
ZwQueryPerformanceCounter(_Out_ PLARGE_INTEGER PerformanceCounter,
                          _Out_ OPTIONAL PLARGE_INTEGER PerformanceFrequency);

NTSTATUS
NTAPI
ZwCreateKeyedEvent(_Out_ PHANDLE KeyedEventHandle,
                   _In_ ACCESS_MASK DesiredAccess,
                   _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
                   _In_ ULONG Flags);
...
NTSTATUS
NTAPI
ZwOpenKeyedEvent(_Out_ PHANDLE KeyedEventHandle, _In_ ACCESS_MASK DesiredAccess,
                 _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwReleaseKeyedEvent(_In_ HANDLE KeyedEventHandle, _In_ PVOID KeyValue,
                    _In_ BOOLEAN Alertable,
                    _In_ OPTIONAL PLARGE_INTEGER Timeout);

NTSTATUS
NTAPI
ZwWaitForKeyedEvent(_In_ HANDLE KeyedEventHandle, _In_ PVOID KeyValue,
                    _In_ BOOLEAN Alertable,
                    _In_ OPTIONAL PLARGE_INTEGER Timeout);
...
NTSTATUS
NTAPI
ZwSetSystemInformation(_In_ SYSTEM_INFORMATION_CLASS SystemInformationClass,
                       _In_ OPTIONAL PVOID SystemInformation,
                       _In_ ULONG SystemInformationLength);

NTSTATUS
NTAPI
ZwSystemDebugControl(_In_ SYSDBG_COMMAND Command,
                     _In_ OPTIONAL PVOID InputBuffer,
                     _In_ ULONG InputBufferLength,
                     _Out_ OPTIONAL PVOID OutputBuffer,
                     _In_ ULONG OutputBufferLength,
...
NTSTATUS
NTAPI
ZwRaiseHardError(_In_ NTSTATUS ErrorStatus, _In_ ULONG NumberOfParameters,
                 _In_ ULONG UnicodeStringParameterMask,
                 _In_ OPTIONAL PULONG_PTR Parameters,
                 _In_ ULONG ValidResponseOptions, _Out_ PULONG Response);
...
NTSTATUS
NTAPI
ZwQueryDefaultLocale(_In_ BOOLEAN UserProfile, _Out_ PLCID DefaultLocaleId);

NTSTATUS
NTAPI
ZwSetDefaultLocale(_In_ BOOLEAN UserProfile, _In_ LCID DefaultLocaleId);

NTSTATUS
NTAPI
ZwQueryInstallUILanguage(_Out_ LANGID * InstallUILanguageId);

NTSTATUS
NTAPI
ZwQueryDefaultUILanguage(_Out_ LANGID * DefaultUILanguageId);

NTSTATUS
NTAPI
ZwSetDefaultUILanguage(_In_ LANGID DefaultUILanguageId);

NTSTATUS
NTAPI
ZwSetDefaultHardErrorPort(_In_ HANDLE DefaultHardErrorPort);

NTSTATUS
NTAPI
ZwShutdownSystem(_In_ SHUTDOWN_ACTION Action);

NTSTATUS
NTAPI
ZwDisplayString(_In_ PUNICODE_STRING String);

NTSTATUS
NTAPI
ZwAddAtom(_In_ OPTIONAL PWSTR AtomName, _In_ ULONG Length,
          _Out_ OPTIONAL PRTL_ATOM Atom);

NTSTATUS
NTAPI
ZwFindAtom(_In_ OPTIONAL PWSTR AtomName, _In_ ULONG Length,
           _Out_ OPTIONAL PRTL_ATOM Atom);

NTSTATUS
NTAPI
ZwDeleteAtom(_In_ RTL_ATOM Atom);

NTSTATUS
NTAPI
ZwQueryInformationAtom(_In_ RTL_ATOM Atom,
                       _In_ ATOM_INFORMATION_CLASS AtomInformationClass,
                       _Out_ OPTIONAL PVOID AtomInformation,
                       _In_ ULONG AtomInformationLength,
                       _Out_ OPTIONAL PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwCancelIoFile(_In_ HANDLE FileHandle, _Out_ PIO_STATUS_BLOCK IoStatusBlock);

NTSTATUS
NTAPI
ZwCreateNamedPipeFile(_Out_ PHANDLE FileHandle, _In_ ULONG DesiredAccess,
                      _In_ POBJECT_ATTRIBUTES ObjectAttributes,
                      _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                      _In_ ULONG ShareAccess, _In_ ULONG CreateDisposition,
                      _In_ ULONG CreateOptions, _In_ ULONG NamedPipeType,
...
NTSTATUS
NTAPI
ZwCreateMailslotFile(_Out_ PHANDLE FileHandle, _In_ ULONG DesiredAccess,
                     _In_ POBJECT_ATTRIBUTES ObjectAttributes,
                     _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                     _In_ ULONG CreateOptions, _In_ ULONG MailslotQuota,
                     _In_ ULONG MaximumMessageSize,
...
NTSTATUS
NTAPI
ZwDeleteFile(_In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwFlushBuffersFile(_In_ HANDLE FileHandle,
                   _Out_ PIO_STATUS_BLOCK IoStatusBlock);

NTSTATUS
NTAPI
ZwNotifyChangeDirectoryFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                            _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                            _In_ OPTIONAL PVOID ApcContext,
                            _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                            _Out_ PVOID Buffer, _In_ ULONG Length,
...
NTSTATUS
NTAPI
ZwQueryAttributesFile(_In_ POBJECT_ATTRIBUTES ObjectAttributes,
                      _Out_ PFILE_BASIC_INFORMATION FileInformation);

NTSTATUS
NTAPI
ZwQueryFullAttributesFile(_In_ POBJECT_ATTRIBUTES ObjectAttributes,
                          _Out_ PFILE_NETWORK_OPEN_INFORMATION FileInformation);

NTSTATUS
NTAPI
ZwQueryEaFile(_In_ HANDLE FileHandle, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
              _Out_ PVOID Buffer, _In_ ULONG Length,
              _In_ BOOLEAN ReturnSingleEntry, _In_ PVOID EaList,
              _In_ ULONG EaListLength, _In_ OPTIONAL PULONG EaIndex OPTIONAL,
              _In_ BOOLEAN RestartScan);
...
NTSTATUS
NTAPI
ZwCreateFile(_Out_ PHANDLE FileHandle, _In_ ACCESS_MASK DesiredAccess,
             _In_ POBJECT_ATTRIBUTES ObjectAttributes,
             _Out_ PIO_STATUS_BLOCK IoStatusBlock,
             _In_ OPTIONAL PLARGE_INTEGER AllocationSize,
             _In_ ULONG FileAttributes, _In_ ULONG ShareAccess,
...
NTSTATUS
NTAPI
ZwDeviceIoControlFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                      _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                      _In_ OPTIONAL PVOID ApcContext,
                      _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                      _In_ ULONG IoControlCode, _In_ OPTIONAL PVOID InputBuffer,
...
NTSTATUS
NTAPI
ZwFsControlFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                _In_ OPTIONAL PVOID ApcContext,
                _Out_ PIO_STATUS_BLOCK IoStatusBlock, _In_ ULONG FsControlCode,
                _In_ OPTIONAL PVOID InputBuffer, _In_ ULONG InputBufferLength,
...
NTSTATUS
NTAPI
ZwLockFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
           _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
           _In_ OPTIONAL PVOID ApcContext, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
           _In_ PLARGE_INTEGER ByteOffset, _In_ PLARGE_INTEGER Length,
           _In_ ULONG Key, _In_ BOOLEAN FailImmediately,
...
NTSTATUS
NTAPI
ZwOpenFile(_Out_ PHANDLE FileHandle, _In_ ACCESS_MASK DesiredAccess,
           _In_ POBJECT_ATTRIBUTES ObjectAttributes,
           _Out_ PIO_STATUS_BLOCK IoStatusBlock, _In_ ULONG ShareAccess,
           _In_ ULONG OpenOptions);
...
NTSTATUS
NTAPI
ZwQueryDirectoryFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                     _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                     _In_ OPTIONAL PVOID ApcContext,
                     _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                     _Out_ PVOID FileInformation, _In_ ULONG Length,
...
NTSTATUS
NTAPI
ZwQueryInformationFile(_In_ HANDLE FileHandle,
                       _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                       _Out_ PVOID FileInformation, _In_ ULONG Length,
                       _In_ FILE_INFORMATION_CLASS FileInformationClass);
...
NTSTATUS
NTAPI
ZwQueryVolumeInformationFile(_In_ HANDLE FileHandle,
                             _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                             _Out_ PVOID FsInformation, _In_ ULONG Length,
                             _In_ FS_INFORMATION_CLASS FsInformationClass);
...
NTSTATUS
NTAPI
ZwReadFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
           _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
           _In_ OPTIONAL PVOID ApcContext, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
           _Out_ PVOID Buffer, _In_ ULONG Length,
           _In_ OPTIONAL PLARGE_INTEGER ByteOffset, _In_ OPTIONAL PULONG Key);
...
NTSTATUS
NTAPI
ZwSetInformationFile(_In_ HANDLE FileHandle,
                     _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                     _In_ PVOID FileInformation, _In_ ULONG Length,
                     _In_ FILE_INFORMATION_CLASS FileInformationClass);
...
NTSTATUS
NTAPI
ZwSetQuotaInformationFile(_In_ HANDLE FileHandle,
                          _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                          _In_ PVOID Buffer, _In_ ULONG Length);

NTSTATUS
NTAPI
ZwSetVolumeInformationFile(_In_ HANDLE FileHandle,
                           _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                           _In_ PVOID FsInformation, _In_ ULONG Length,
                           _In_ FS_INFORMATION_CLASS FsInformationClass);
...
NTSTATUS
NTAPI
ZwWriteFile(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
            _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
            _In_ OPTIONAL PVOID ApcContext,
            _Out_ PIO_STATUS_BLOCK IoStatusBlock, _In_ PVOID Buffer,
            _In_ ULONG Length, _In_ OPTIONAL PLARGE_INTEGER ByteOffset,
...
NTSTATUS
NTAPI
ZwUnlockFile(_In_ HANDLE FileHandle, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
             _In_ PLARGE_INTEGER ByteOffset, _In_ PLARGE_INTEGER Length,
             _In_ ULONG Key);

NTSTATUS
NTAPI
ZwReadFileScatter(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                  _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                  _In_ OPTIONAL PVOID ApcContext,
                  _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                  _In_ PFILE_SEGMENT_ELEMENT SegmentArray, _In_ ULONG Length,
...
NTSTATUS
NTAPI
ZwSetEaFile(_In_ HANDLE FileHandle, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
            _In_ PVOID Buffer, _In_ ULONG Length);

NTSTATUS
NTAPI
ZwWriteFileGather(_In_ HANDLE FileHandle, _In_ OPTIONAL HANDLE Event,
                  _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                  _In_ OPTIONAL PVOID ApcContext,
                  _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                  _In_ PFILE_SEGMENT_ELEMENT SegmentArray, _In_ ULONG Length,
...
NTSTATUS
NTAPI
ZwLoadDriver(_In_ PUNICODE_STRING DriverServiceName);

NTSTATUS
NTAPI
ZwUnloadDriver(_In_ PUNICODE_STRING DriverServiceName);

NTSTATUS
NTAPI
ZwCreateIoCompletion(_Out_ PHANDLE IoCompletionHandle,
                     _In_ ACCESS_MASK DesiredAccess,
                     _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
                     _In_ ULONG Count OPTIONAL);
...
NTSTATUS
NTAPI
ZwOpenIoCompletion(_Out_ PHANDLE IoCompletionHandle,
                   _In_ ACCESS_MASK DesiredAccess,
                   _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwQueryIoCompletion(_In_ HANDLE IoCompletionHandle,
                    _In_ IO_COMPLETION_INFORMATION_CLASS
                    IoCompletionInformationClass,
                    _Out_ PVOID IoCompletionInformation,
                    _In_ ULONG IoCompletionInformationLength,
...
NTSTATUS
NTAPI
ZwSetIoCompletion(_In_ HANDLE IoCompletionHandle, _In_ PVOID KeyContext,
                  _In_ OPTIONAL PVOID ApcContext, _In_ NTSTATUS IoStatus,
                  _In_ ULONG_PTR IoStatusInformation);

NTSTATUS
NTAPI
ZwRemoveIoCompletion(_In_ HANDLE IoCompletionHandle, _Out_ PVOID * KeyContext,
                     _Out_ PVOID *                                 ApcContext,
                     _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                     _In_ OPTIONAL PLARGE_INTEGER Timeout);
...
NTSTATUS
NTAPI
ZwCallbackReturn(_In_ PVOID OutputBuffer OPTIONAL, _In_ ULONG OutputLength,
                 _In_ NTSTATUS Status);

NTSTATUS
NTAPI
ZwQueryDebugFilterState(_In_ ULONG ComponentId, _In_ ULONG Level);

NTSTATUS
NTAPI
ZwSetDebugFilterState(_In_ ULONG ComponentId, _In_ ULONG Level,
                      _In_ BOOLEAN State);
...
NTSTATUS
NTAPI
ZwCreatePort(_Out_ PHANDLE PortHandle, _In_ POBJECT_ATTRIBUTES ObjectAttributes,
             _In_ ULONG MaxConnectionInfoLength, _In_ ULONG MaxMessageLength,
             _In_ OPTIONAL ULONG MaxPoolUsage);

NTSTATUS
NTAPI
ZwCreateWaitablePort(_Out_ PHANDLE PortHandle,
                     _In_ POBJECT_ATTRIBUTES ObjectAttributes,
                     _In_ ULONG MaxConnectionInfoLength,
                     _In_ ULONG MaxMessageLength,
                     _In_ OPTIONAL ULONG MaxPoolUsage);
...
NTSTATUS
NTAPI
ZwConnectPort(_Out_ PHANDLE PortHandle, _In_ PUNICODE_STRING PortName,
              _In_ PSECURITY_QUALITY_OF_SERVICE SecurityQos,
              _In_ _Out_ OPTIONAL PPORT_VIEW ClientView,
              _In_ _Out_ OPTIONAL PREMOTE_PORT_VIEW ServerView,
              _Out_ OPTIONAL PULONG MaxMessageLength,
...
NTSTATUS
NTAPI
ZwSecureConnectPort(_Out_ PHANDLE PortHandle, _In_ PUNICODE_STRING PortName,
                    _In_ PSECURITY_QUALITY_OF_SERVICE SecurityQos,
                    _In_ _Out_ OPTIONAL PPORT_VIEW ClientView,
                    _In_ OPTIONAL PSID RequiredServerSid,
                    _In_ _Out_ OPTIONAL PREMOTE_PORT_VIEW ServerView,
...
NTSTATUS
NTAPI
ZwListenPort(_In_ HANDLE PortHandle, _Out_ PPORT_MESSAGE ConnectionRequest);

NTSTATUS
NTAPI
ZwAcceptConnectPort(_Out_ PHANDLE PortHandle, _In_ OPTIONAL PVOID PortContext,
                    _In_ PPORT_MESSAGE ConnectionRequest,
                    _In_ BOOLEAN AcceptConnection,
                    _In_ _Out_ OPTIONAL PPORT_VIEW ServerView,
                    _Out_ OPTIONAL PREMOTE_PORT_VIEW ClientView);
...
NTSTATUS
NTAPI
ZwCompleteConnectPort(_In_ HANDLE PortHandle);

NTSTATUS
NTAPI
ZwRequestPort(_In_ HANDLE PortHandle, _In_ PPORT_MESSAGE RequestMessage);

NTSTATUS
NTAPI
ZwRequestWaitReplyPort(_In_ HANDLE PortHandle,
                       _In_ PPORT_MESSAGE RequestMessage,
                       _Out_ PPORT_MESSAGE ReplyMessage);

NTSTATUS
NTAPI
ZwReplyPort(_In_ HANDLE PortHandle, _In_ PPORT_MESSAGE ReplyMessage);

NTSTATUS
NTAPI
ZwReplyWaitReplyPort(_In_ HANDLE PortHandle,
                     _In_ _Out_ PPORT_MESSAGE ReplyMessage);

NTSTATUS
NTAPI
ZwReplyWaitReceivePort(_In_ HANDLE PortHandle,
                       _Out_ OPTIONAL PVOID * PortContext,
                       _In_ OPTIONAL PPORT_MESSAGE ReplyMessage,
                       _Out_ PPORT_MESSAGE ReceiveMessage);
...
NTSTATUS
NTAPI
ZwReplyWaitReceivePortEx(_In_ HANDLE PortHandle,
                         _Out_ OPTIONAL PVOID * PortContext,
                         _In_ OPTIONAL PPORT_MESSAGE ReplyMessage,
                         _Out_ PPORT_MESSAGE ReceiveMessage,
                         _In_ OPTIONAL PLARGE_INTEGER Timeout);
...
NTSTATUS
NTAPI
ZwImpersonateClientOfPort(_In_ HANDLE PortHandle, _In_ PPORT_MESSAGE Message);

NTSTATUS
NTAPI
ZwReadRequestData(_In_ HANDLE PortHandle, _In_ PPORT_MESSAGE Message,
                  _In_ ULONG DataEntryIndex, _Out_ PVOID Buffer,
                  _In_ SIZE_T BufferSize,
                  _Out_ OPTIONAL PSIZE_T NumberOfBytesRead);
...
NTSTATUS
NTAPI
ZwWriteRequestData(_In_ HANDLE PortHandle, _In_ PPORT_MESSAGE Message,
                   _In_ ULONG DataEntryIndex, _In_ PVOID Buffer,
                   _In_ SIZE_T BufferSize,
                   _Out_ OPTIONAL PSIZE_T NumberOfBytesWritten);
...
NTSTATUS
NTAPI
ZwQueryInformationPort(_In_ HANDLE PortHandle,
                       _In_ PORT_INFORMATION_CLASS PortInformationClass,
                       _Out_ PVOID PortInformation, _In_ ULONG Length,
                       _Out_ OPTIONAL PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwOpenSection(_Out_ PHANDLE SectionHandle, _In_ ACCESS_MASK DesiredAccess,
              _In_ POBJECT_ATTRIBUTES ObjectAttributes);
...
NTSTATUS
NTAPI
ZwUnmapViewOfSection(_In_ HANDLE ProcessHandle, _In_ PVOID BaseAddress);

NTSTATUS
NTAPI
ZwExtendSection(_In_ HANDLE SectionHandle,
                _In_ _Out_ PLARGE_INTEGER NewSectionSize);

NTSTATUS
NTAPI
ZwAreMappedFilesTheSame(_In_ PVOID File1MappedAsAnImage,
                        _In_ PVOID File2MappedAsFile);

NTSTATUS
NTAPI
ZwAllocateVirtualMemory(_In_ HANDLE ProcessHandle,
                        _In_ _Out_ PVOID * BaseAddress, _In_ ULONG_PTR ZeroBits,
                        _In_ _Out_ PSIZE_T RegionSize,
                        _In_ ULONG AllocationType, _In_ ULONG Protect);
...
NTSTATUS
NTAPI
ZwFreeVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                    _In_ _Out_ PSIZE_T RegionSize, _In_ ULONG FreeType);

NTSTATUS
NTAPI
ZwReadVirtualMemory(_In_ HANDLE ProcessHandle, _In_ OPTIONAL PVOID BaseAddress,
                    _Out_ PVOID Buffer, _In_ SIZE_T BufferSize,
                    _Out_ OPTIONAL PSIZE_T NumberOfBytesRead);

NTSTATUS
NTAPI
ZwWriteVirtualMemory(_In_ HANDLE ProcessHandle, _In_ OPTIONAL PVOID BaseAddress,
                     _In_ CONST VOID * Buffer, _In_ SIZE_T BufferSize,
                     _Out_ OPTIONAL PSIZE_T NumberOfBytesWritten);

NTSTATUS
NTAPI
ZwFlushVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                     _In_ _Out_ PSIZE_T RegionSize,
                     _Out_ PIO_STATUS_BLOCK IoStatus);

NTSTATUS
NTAPI
ZwLockVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                    _In_ _Out_ PSIZE_T RegionSize, _In_ ULONG MapType);

NTSTATUS
NTAPI
ZwUnlockVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                      _In_ _Out_ PSIZE_T RegionSize, _In_ ULONG MapType);

NTSTATUS
NTAPI
ZwProtectVirtualMemory(_In_ HANDLE ProcessHandle, _In_ _Out_ PVOID * BaseAddress,
                       _In_ _Out_ PSIZE_T RegionSize, _In_ ULONG NewProtect,
                       _Out_ PULONG OldProtect);

NTSTATUS
NTAPI
ZwQueryVirtualMemory(_In_ HANDLE ProcessHandle, _In_ PVOID BaseAddress,
                     _In_ MEMORY_INFORMATION_CLASS MemoryInformationClass,
                     _Out_ PVOID MemoryInformation,
                     _In_ SIZE_T MemoryInformationLength,
                     _Out_ OPTIONAL PSIZE_T ReturnLength);
...
NTSTATUS
NTAPI
ZwQuerySection(_In_ HANDLE SectionHandle,
               _In_ SECTION_INFORMATION_CLASS SectionInformationClass,
               _Out_ PVOID SectionInformation,
               _In_ SIZE_T SectionInformationLength,
               _Out_ OPTIONAL PSIZE_T ReturnLength);
...
NTSTATUS
NTAPI
ZwMapUserPhysicalPages(_In_ PVOID VirtualAddress, _In_ ULONG_PTR NumberOfPages,
                       _In_ OPTIONAL PULONG_PTR UserPfnArray);

NTSTATUS
NTAPI
ZwMapUserPhysicalPagesScatter(_In_ PVOID * VirtualAddresses,
                              _In_ ULONG_PTR NumberOfPages,
                              _In_ OPTIONAL PULONG_PTR UserPfnArray);

NTSTATUS
NTAPI
ZwAllocateUserPhysicalPages(_In_ HANDLE ProcessHandle,
                            _In_ _Out_ PULONG_PTR NumberOfPages,
                            _Out_ PULONG_PTR UserPfnArray);

NTSTATUS
NTAPI
ZwFreeUserPhysicalPages(_In_ HANDLE ProcessHandle,
                        _In_ _Out_ PULONG_PTR NumberOfPages,
                        _In_ PULONG_PTR UserPfnArray);

NTSTATUS
NTAPI
ZwGetWriteWatch(_In_ HANDLE ProcessHandle, _In_ ULONG Flags,
                _In_ PVOID BaseAddress, _In_ SIZE_T RegionSize,
                _Out_ PVOID * UserAddressArray,
                _In_ _Out_ PULONG_PTR EntriesInUserAddressArray,
                _Out_ PULONG Granularity);
...
NTSTATUS
NTAPI
ZwResetWriteWatch(_In_ HANDLE ProcessHandle, _In_ PVOID BaseAddress,
                  _In_ SIZE_T RegionSize);

NTSTATUS
NTAPI
ZwCreatePagingFile(_In_ PUNICODE_STRING PageFileName,
                   _In_ PLARGE_INTEGER MinimumSize,
                   _In_ PLARGE_INTEGER MaximumSize, _In_ ULONG Priority);

NTSTATUS
NTAPI
ZwFlushInstructionCache(_In_ HANDLE ProcessHandle,
                        _In_ OPTIONAL PVOID BaseAddress, _In_ SIZE_T Length);
...
NTSTATUS
NTAPI
ZwQueryObject(_In_ HANDLE Handle,
              _In_ OBJECT_INFORMATION_CLASS ObjectInformationClass,
              _Out_ PVOID ObjectInformation, _In_ ULONG ObjectInformationLength,
              _Out_ PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwSetInformationObject(_In_ HANDLE Handle,
                       _In_ OBJECT_INFORMATION_CLASS ObjectInformationClass,
                       _In_ PVOID ObjectInformation,
                       _In_ ULONG ObjectInformationLength);
...
NTSTATUS
NTAPI
ZwDuplicateObject(_In_ HANDLE SourceProcessHandle, _In_ HANDLE SourceHandle,
                  _In_ OPTIONAL HANDLE TargetProcessHandle,
                  _Out_ PHANDLE TargetHandle, _In_ ACCESS_MASK DesiredAccess,
                  _In_ ULONG HandleAttributes, _In_ ULONG Options);
...
NTSTATUS
NTAPI
ZwMakeTemporaryObject(_In_ HANDLE Handle);

NTSTATUS
NTAPI
ZwMakePermanentObject(_In_ HANDLE Handle);

NTSTATUS
NTAPI
ZwSignalAndWaitForSingleObject(_In_ HANDLE SignalHandle, _In_ HANDLE WaitHandle,
                               _In_ BOOLEAN Alertable,
                               _In_ OPTIONAL PLARGE_INTEGER Timeout);

NTSTATUS
NTAPI
ZwWaitForSingleObject(_In_ HANDLE Handle, _In_ BOOLEAN Alertable,
                      _In_ OPTIONAL PLARGE_INTEGER Timeout);

NTSTATUS
NTAPI
ZwWaitForMultipleObjects(_In_ ULONG Count, _In_ HANDLE Handles[],
                         _In_ WAIT_TYPE WaitType, _In_ BOOLEAN Alertable,
                         _In_ OPTIONAL PLARGE_INTEGER Timeout);

NTSTATUS
NTAPI
ZwWaitForMultipleObjects32(_In_ ULONG Count, _In_ LONG Handles[],
                           _In_ WAIT_TYPE WaitType, _In_ BOOLEAN Alertable,
                           _In_ OPTIONAL PLARGE_INTEGER Timeout);

NTSTATUS
NTAPI
ZwSetSecurityObject(_In_ HANDLE Handle,
                    _In_ SECURITY_INFORMATION SecurityInformation,
                    _In_ PSECURITY_DESCRIPTOR SecurityDescriptor);

NTSTATUS
NTAPI
ZwQuerySecurityObject(_In_ HANDLE Handle,
                      _In_ SECURITY_INFORMATION SecurityInformation,
                      _Out_ PSECURITY_DESCRIPTOR SecurityDescriptor,
                      _In_ ULONG Length, _Out_ PULONG LengthNeeded);
...
NTSTATUS
NTAPI
ZwCreateDirectoryObject(_Out_ PHANDLE DirectoryHandle,
                        _In_ ACCESS_MASK DesiredAccess,
                        _In_ POBJECT_ATTRIBUTES ObjectAttributes);
...
NTSTATUS
NTAPI
ZwQueryDirectoryObject(_In_ HANDLE DirectoryHandle, _Out_ PVOID Buffer,
                       _In_ ULONG Length, _In_ BOOLEAN ReturnSingleEntry,
                       _In_ BOOLEAN RestartScan, _In_ _Out_ PULONG Context,
                       _Out_ PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwCreateSymbolicLinkObject(_Out_ PHANDLE LinkHandle,
                           _In_ ACCESS_MASK DesiredAccess,
                           _In_ POBJECT_ATTRIBUTES ObjectAttributes,
                           _In_ PUNICODE_STRING LinkTarget);
...
NTSTATUS
NTAPI
ZwOpenSymbolicLinkObject(_Out_ PHANDLE LinkHandle,
                         _In_ ACCESS_MASK DesiredAccess,
                         _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwQuerySymbolicLinkObject(_In_ HANDLE LinkHandle,
                          _In_ _Out_ PUNICODE_STRING LinkTarget,
                          _Out_ PULONG ReturnedLength);

NTSTATUS
NTAPI
ZwGetPlugPlayEvent(_In_ HANDLE EventHandle, _In_ OPTIONAL PVOID Context,
                   _Out_ PPLUGPLAY_EVENT_BLOCK EventBlock,
                   _In_ ULONG EventBufferSize);

NTSTATUS
NTAPI
ZwPlugPlayControl(_In_ PLUGPLAY_CONTROL_CLASS PnPControlClass,
                  _In_ _Out_ PVOID PnPControlData,
                  _In_ ULONG PnPControlDataLength);

NTSTATUS
NTAPI
ZwPowerInformation(_In_ POWER_INFORMATION_LEVEL InformationLevel,
                   _In_ OPTIONAL PVOID InputBuffer,
                   _In_ ULONG InputBufferLength,
                   _Out_ OPTIONAL PVOID OutputBuffer,
                   _In_ ULONG OutputBufferLength);
...
NTSTATUS
NTAPI
ZwSetThreadExecutionState(_In_ EXECUTION_STATE esFlags,
                          _Out_ EXECUTION_STATE * PreviousFlags);

NTSTATUS
NTAPI
ZwRequestWakeupLatency(_In_ LATENCY_TIME latency);

NTSTATUS
NTAPI
ZwCancelDeviceWakeupRequest(_In_ HANDLE Device);

NTSTATUS
NTAPI
ZwRequestDeviceWakeup(_In_ HANDLE Device);

NTSTATUS
NTAPI
ZwCreateProcess(_Out_ PHANDLE ProcessHandle, _In_ ACCESS_MASK DesiredAccess,
                _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
                _In_ HANDLE ParentProcess, _In_ BOOLEAN InheritObjectTable,
                _In_ OPTIONAL HANDLE SectionHandle,
                _In_ OPTIONAL HANDLE DebugPort,
...
NTSTATUS
NTAPI
ZwCreateProcessEx(_Out_ PHANDLE ProcessHandle, _In_ ACCESS_MASK DesiredAccess,
                  _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
                  _In_ HANDLE ParentProcess, _In_ ULONG Flags,
                  _In_ OPTIONAL HANDLE SectionHandle,
                  _In_ OPTIONAL HANDLE DebugPort,
...
NTSTATUS
NTAPI
ZwOpenProcess(_Out_ PHANDLE ProcessHandle, _In_ ACCESS_MASK DesiredAccess,
              _In_ POBJECT_ATTRIBUTES ObjectAttributes,
              _In_ OPTIONAL PCLIENT_ID ClientId);

NTSTATUS
NTAPI
ZwTerminateProcess(_In_ OPTIONAL HANDLE ProcessHandle,
                   _In_ NTSTATUS ExitStatus);
...
NTSTATUS
NTAPI
ZwGetNextProcess(_In_ HANDLE ProcessHandle, _In_ ACCESS_MASK DesiredAccess,
                 _In_ ULONG HandleAttributes, _In_ ULONG Flags,
                 _Out_ PHANDLE NewProcessHandle);

NTSTATUS
NTAPI
ZwGetNextThread(_In_ HANDLE ProcessHandle, _In_ HANDLE ThreadHandle,
                _In_ ACCESS_MASK DesiredAccess, _In_ ULONG HandleAttributes,
                _In_ ULONG Flags, _Out_ PHANDLE NewThreadHandle);
...
NTSTATUS
NTAPI
ZwSetInformationProcess(_In_ HANDLE ProcessHandle,
                        _In_ PROCESSINFOCLASS ProcessInformationClass,
                        _In_ PVOID ProcessInformation,
                        _In_ ULONG ProcessInformationLength);
...
NTSTATUS
NTAPI
ZwCreateThread(_Out_ PHANDLE ThreadHandle, _In_ ACCESS_MASK DesiredAccess,
               _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
               _In_ HANDLE ProcessHandle, _Out_ PCLIENT_ID ClientId,
               _In_ PCONTEXT ThreadContext, _In_ PINITIAL_TEB InitialTeb,
               _In_ BOOLEAN CreateSuspended);
...
NTSTATUS
NTAPI
ZwOpenThread(_Out_ PHANDLE ThreadHandle, _In_ ACCESS_MASK DesiredAccess,
             _In_ POBJECT_ATTRIBUTES ObjectAttributes,
             _In_ OPTIONAL PCLIENT_ID ClientId);

NTSTATUS
NTAPI
ZwTerminateThread(_In_ OPTIONAL HANDLE ThreadHandle, _In_ NTSTATUS ExitStatus);

NTSTATUS
NTAPI
ZwSuspendThread(_In_ HANDLE ThreadHandle,
                _Out_ OPTIONAL PULONG PreviousSuspendCount);

NTSTATUS
NTAPI
ZwResumeThread(_In_ HANDLE ThreadHandle,
               _Out_ OPTIONAL PULONG PreviousSuspendCount);

NTSTATUS
NTAPI
ZwSuspendProcess(_In_ HANDLE ProcessHandle);

NTSTATUS
NTAPI
ZwResumeProcess(_In_ HANDLE ProcessHandle);

NTSTATUS
NTAPI
ZwGetContextThread(_In_ HANDLE ThreadHandle, _In_ _Out_ PCONTEXT ThreadContext);

NTSTATUS
NTAPI
ZwSetContextThread(_In_ HANDLE ThreadHandle, _In_ PCONTEXT ThreadContext);

NTSTATUS
NTAPI
ZwQueryInformationThread(_In_ HANDLE ThreadHandle,
                         _In_ THREADINFOCLASS ThreadInformationClass,
                         _Out_ PVOID ThreadInformation,
                         _In_ ULONG ThreadInformationLength,
                         _Out_ OPTIONAL PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwSetInformationThread(_In_ HANDLE ThreadHandle,
                       _In_ THREADINFOCLASS ThreadInformationClass,
                       _In_ PVOID ThreadInformation,
                       _In_ ULONG ThreadInformationLength);
...
NTSTATUS
NTAPI
ZwAlertThread(_In_ HANDLE ThreadHandle);

NTSTATUS
NTAPI
ZwAlertResumeThread(_In_ HANDLE ThreadHandle,
                    _Out_ OPTIONAL PULONG PreviousSuspendCount);

NTSTATUS
NTAPI
ZwImpersonateThread(_In_ HANDLE ServerThreadHandle,
                    _In_ HANDLE ClientThreadHandle,
                    _In_ PSECURITY_QUALITY_OF_SERVICE SecurityQos);
...
NTSTATUS
NTAPI
ZwRegisterThreadTerminatePort(_In_ HANDLE PortHandle);

NTSTATUS
NTAPI
ZwSetLdtEntries(_In_ ULONG Selector0, _In_ ULONG Entry0Low, _In_ ULONG Entry0Hi,
                _In_ ULONG Selector1, _In_ ULONG Entry1Low,
                _In_ ULONG Entry1Hi);

NTSTATUS
NTAPI
ZwQueueApcThread(_In_ HANDLE ThreadHandle, _In_ PPS_APC_ROUTINE ApcRoutine,
                 _In_ OPTIONAL PVOID ApcArgument1,
                 _In_ OPTIONAL PVOID ApcArgument2,
                 _In_ OPTIONAL PVOID ApcArgument3);
...
NTSTATUS
NTAPI
ZwCreateJobObject(_Out_ PHANDLE JobHandle, _In_ ACCESS_MASK DesiredAccess,
                  _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwOpenJobObject(_Out_ PHANDLE JobHandle, _In_ ACCESS_MASK DesiredAccess,
                _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwAssignProcessToJobObject(_In_ HANDLE JobHandle, _In_ HANDLE ProcessHandle);

NTSTATUS
NTAPI
ZwTerminateJobObject(_In_ HANDLE JobHandle, _In_ NTSTATUS ExitStatus);

NTSTATUS
NTAPI
ZwIsProcessInJob(_In_ HANDLE ProcessHandle, _In_ OPTIONAL HANDLE JobHandle);

NTSTATUS
NTAPI
ZwCreateJobSet(_In_ ULONG NumJob, _In_ PJOB_SET_ARRAY UserJobSet,
               _In_ ULONG Flags);

NTSTATUS
NTAPI
ZwQueryInformationJobObject(_In_ OPTIONAL HANDLE JobHandle,
                            _In_ JOBOBJECTINFOCLASS JobObjectInformationClass,
                            _Out_ PVOID JobObjectInformation,
                            _In_ ULONG JobObjectInformationLength,
                            _Out_ OPTIONAL PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwSetInformationJobObject(_In_ HANDLE JobHandle,
                          _In_ JOBOBJECTINFOCLASS JobObjectInformationClass,
                          _In_ PVOID JobObjectInformation,
                          _In_ ULONG JobObjectInformationLength);
...
NTSTATUS
NTAPI
ZwCreateKey(_Out_ PHANDLE KeyHandle, _In_ ACCESS_MASK DesiredAccess,
            _In_ POBJECT_ATTRIBUTES ObjectAttributes, ULONG TitleIndex,
            _In_ OPTIONAL PUNICODE_STRING Class, _In_ ULONG CreateOptions,
            _Out_ OPTIONAL PULONG Disposition);
...
NTSTATUS
NTAPI
ZwDeleteKey(_In_ HANDLE KeyHandle);

NTSTATUS
NTAPI
ZwDeleteValueKey(_In_ HANDLE KeyHandle, _In_ PUNICODE_STRING ValueName);

NTSTATUS
NTAPI
ZwEnumerateKey(_In_ HANDLE KeyHandle, _In_ ULONG Index,
               _In_ KEY_INFORMATION_CLASS KeyInformationClass,
               _Out_ OPTIONAL PVOID KeyInformation, _In_ ULONG Length,
               _Out_ PULONG ResultLength);
...
NTSTATUS
NTAPI
ZwEnumerateValueKey(_In_ HANDLE KeyHandle, _In_ ULONG Index,
                    _In_ KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass,
                    _Out_ OPTIONAL PVOID KeyValueInformation, _In_ ULONG Length,
                    _Out_ PULONG ResultLength);
...
NTSTATUS
NTAPI
ZwFlushKey(_In_ HANDLE KeyHandle);

NTSTATUS
NTAPI
ZwInitializeRegistry(_In_ USHORT BootCondition);

NTSTATUS
NTAPI
ZwNotifyChangeKey(_In_ HANDLE KeyHandle, _In_ OPTIONAL HANDLE Event,
                  _In_ OPTIONAL PIO_APC_ROUTINE ApcRoutine,
                  _In_ OPTIONAL PVOID ApcContext,
                  _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                  _In_ ULONG CompletionFilter, _In_ BOOLEAN WatchTree,
...
NTSTATUS
NTAPI
ZwLoadKey(_In_ POBJECT_ATTRIBUTES TargetKey,
          _In_ POBJECT_ATTRIBUTES SourceFile);

NTSTATUS
NTAPI
ZwLoadKey2(_In_ POBJECT_ATTRIBUTES TargetKey,
           _In_ POBJECT_ATTRIBUTES SourceFile, _In_ ULONG Flags);

NTSTATUS
NTAPI
ZwLoadKeyEx(_In_ POBJECT_ATTRIBUTES TargetKey,
            _In_ POBJECT_ATTRIBUTES SourceFile, _In_ ULONG Flags,
            _In_ OPTIONAL HANDLE TrustClassKey);

NTSTATUS
NTAPI
ZwOpenKey(_Out_ PHANDLE KeyHandle, _In_ ACCESS_MASK DesiredAccess,
          _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
ZwQueryKey(_In_ HANDLE KeyHandle,
           _In_ KEY_INFORMATION_CLASS KeyInformationClass,
           _Out_ OPTIONAL PVOID KeyInformation, _In_ ULONG Length,
           _Out_ PULONG ResultLength);
...
NTSTATUS
NTAPI
ZwQueryValueKey(_In_ HANDLE KeyHandle, _In_ PUNICODE_STRING ValueName,
                _In_ KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass,
                _Out_ OPTIONAL PVOID KeyValueInformation, _In_ ULONG Length,
                _Out_ PULONG ResultLength);
...
NTSTATUS
NTAPI
ZwQueryMultipleValueKey(_In_ HANDLE KeyHandle,
                        _In_ _Out_ PKEY_VALUE_ENTRY ValueEntries,
                        _In_ ULONG EntryCount, _Out_ PVOID ValueBuffer,
                        _In_ _Out_ PULONG BufferLength,
                        _Out_ OPTIONAL PULONG RequiredBufferLength);
...
NTSTATUS
NTAPI
ZwReplaceKey(_In_ POBJECT_ATTRIBUTES NewFile, _In_ HANDLE TargetHandle,
             _In_ POBJECT_ATTRIBUTES OldFile);

NTSTATUS
NTAPI
ZwRenameKey(_In_ HANDLE KeyHandle, _In_ PUNICODE_STRING NewName);

NTSTATUS
NTAPI
ZwCompactKeys(_In_ ULONG Count, _In_ HANDLE KeyArray[]);

NTSTATUS
NTAPI
ZwCompressKey(_In_ HANDLE Key);

NTSTATUS
NTAPI
ZwRestoreKey(_In_ HANDLE KeyHandle, _In_ HANDLE FileHandle, _In_ ULONG Flags);

NTSTATUS
NTAPI
ZwSaveKey(_In_ HANDLE KeyHandle, _In_ HANDLE FileHandle);

NTSTATUS
NTAPI
ZwSaveKeyEx(_In_ HANDLE KeyHandle, _In_ HANDLE FileHandle, _In_ ULONG Format);

NTSTATUS
NTAPI
ZwSaveMergedKeys(_In_ HANDLE HighPrecedenceKeyHandle,
                 _In_ HANDLE LowPrecedenceKeyHandle, _In_ HANDLE FileHandle);

NTSTATUS
NTAPI
ZwSetValueKey(_In_ HANDLE KeyHandle, _In_ PUNICODE_STRING ValueName,
              _In_ OPTIONAL ULONG TitleIndex, _In_ ULONG Type,
              _In_ OPTIONAL PVOID Data, _In_ ULONG DataSize);

NTSTATUS
NTAPI
ZwUnloadKey(_In_ POBJECT_ATTRIBUTES TargetKey);

NTSTATUS
NTAPI
ZwUnloadKey2(_In_ POBJECT_ATTRIBUTES TargetKey, _In_ ULONG Flags);

NTSTATUS
NTAPI
ZwUnloadKeyEx(_In_ POBJECT_ATTRIBUTES TargetKey, _In_ OPTIONAL HANDLE Event);

NTSTATUS
NTAPI
ZwSetInformationKey(_In_ HANDLE KeyHandle,
                    _In_ KEY_SET_INFORMATION_CLASS KeySetInformationClass,
                    _In_ PVOID KeySetInformation,
                    _In_ ULONG KeySetInformationLength);
...
NTSTATUS
NTAPI
ZwQueryOpenSubKeys(_In_ POBJECT_ATTRIBUTES TargetKey, _Out_ PULONG HandleCount);

NTSTATUS
NTAPI
ZwQueryOpenSubKeysEx(_In_ POBJECT_ATTRIBUTES TargetKey, _In_ ULONG BufferLength,
                     _Out_ PVOID Buffer, _Out_ PULONG RequiredSize);

NTSTATUS
NTAPI
ZwLockRegistryKey(_In_ HANDLE KeyHandle);

NTSTATUS
NTAPI
ZwLockProductActivationKeys(_In_ _Out_ OPTIONAL ULONG * pPrivateVer,
                            _Out_ OPTIONAL ULONG *      pSafeMode);

NTSTATUS
NTAPI
ZwAccessCheck(_In_ PSECURITY_DESCRIPTOR SecurityDescriptor,
              _In_ HANDLE ClientToken, _In_ ACCESS_MASK DesiredAccess,
              _In_ PGENERIC_MAPPING GenericMapping,
              _Out_ PPRIVILEGE_SET PrivilegeSet,
              _In_ _Out_ PULONG PrivilegeSetLength,
...
NTSTATUS
NTAPI
ZwCreateToken(_Out_ PHANDLE TokenHandle, _In_ ACCESS_MASK DesiredAccess,
              _In_ OPTIONAL POBJECT_ATTRIBUTES ObjectAttributes,
              _In_ TOKEN_TYPE TokenType, _In_ PLUID AuthenticationId,
              _In_ PLARGE_INTEGER ExpirationTime, _In_ PTOKEN_USER User,
              _In_ PTOKEN_GROUPS Groups, _In_ PTOKEN_PRIVILEGES Privileges,
...
NTSTATUS
NTAPI
ZwCompareTokens(_In_ HANDLE FirstTokenHandle, _In_ HANDLE SecondTokenHandle,
                _Out_ PBOOLEAN Equal);

NTSTATUS
NTAPI
ZwOpenThreadToken(_In_ HANDLE ThreadHandle, _In_ ACCESS_MASK DesiredAccess,
                  _In_ BOOLEAN OpenAsSelf, _Out_ PHANDLE TokenHandle);

NTSTATUS
NTAPI
ZwOpenThreadTokenEx(_In_ HANDLE ThreadHandle, _In_ ACCESS_MASK DesiredAccess,
                    _In_ BOOLEAN OpenAsSelf, _In_ ULONG HandleAttributes,
                    _Out_ PHANDLE TokenHandle);

NTSTATUS
NTAPI
ZwOpenProcessToken(_In_ HANDLE ProcessHandle, _In_ ACCESS_MASK DesiredAccess,
                   _Out_ PHANDLE TokenHandle);

NTSTATUS
NTAPI
ZwOpenProcessTokenEx(_In_ HANDLE ProcessHandle, _In_ ACCESS_MASK DesiredAccess,
                     _In_ ULONG HandleAttributes, _Out_ PHANDLE TokenHandle);

NTSTATUS
NTAPI
ZwDuplicateToken(_In_ HANDLE ExistingTokenHandle,
                 _In_ ACCESS_MASK DesiredAccess,
                 _In_ POBJECT_ATTRIBUTES ObjectAttributes,
                 _In_ BOOLEAN EffectiveOnly, _In_ TOKEN_TYPE TokenType,
                 _Out_ PHANDLE NewTokenHandle);
...
NTSTATUS
NTAPI
ZwFilterToken(_In_ HANDLE ExistingTokenHandle, _In_ ULONG Flags,
              _In_ OPTIONAL PTOKEN_GROUPS SidsToDisable,
              _In_ OPTIONAL PTOKEN_PRIVILEGES PrivilegesToDelete,
              _In_ OPTIONAL PTOKEN_GROUPS RestrictedSids,
              _Out_ PHANDLE NewTokenHandle);
...
NTSTATUS
NTAPI
ZwImpersonateAnonymousToken(_In_ HANDLE ThreadHandle);

NTSTATUS
NTAPI
ZwQueryInformationToken(_In_ HANDLE TokenHandle,
                        _In_ TOKEN_INFORMATION_CLASS TokenInformationClass,
                        _Out_ PVOID TokenInformation,
                        _In_ ULONG TokenInformationLength,
                        _Out_ PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwSetInformationToken(_In_ HANDLE TokenHandle,
                      _In_ TOKEN_INFORMATION_CLASS TokenInformationClass,
                      _In_ PVOID TokenInformation,
                      _In_ ULONG TokenInformationLength);
...
NTSTATUS
NTAPI
ZwAdjustPrivilegesToken(_In_ HANDLE TokenHandle,
                        _In_ BOOLEAN DisableAllPrivileges,
                        _In_ OPTIONAL PTOKEN_PRIVILEGES NewState,
                        _In_ OPTIONAL ULONG BufferLength,
                        _Out_ PTOKEN_PRIVILEGES PreviousState,
...
NTSTATUS
NTAPI
ZwAdjustGroupsToken(_In_ HANDLE TokenHandle, _In_ BOOLEAN ResetToDefault,
                    _In_ PTOKEN_GROUPS NewState,
                    _In_ OPTIONAL ULONG BufferLength,
                    _Out_ PTOKEN_GROUPS PreviousState,
                    _Out_ PULONG ReturnLength);
...
NTSTATUS
NTAPI
ZwPrivilegeCheck(_In_ HANDLE ClientToken,
                 _In_ _Out_ PPRIVILEGE_SET RequiredPrivileges,
                 _Out_ PBOOLEAN Result);
...
NTSTATUS
NTAPI
ZwOpenObjectAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                       _In_ OPTIONAL PVOID HandleId,
                       _In_ PUNICODE_STRING ObjectTypeName,
                       _In_ PUNICODE_STRING ObjectName,
                       _In_ OPTIONAL PSECURITY_DESCRIPTOR SecurityDescriptor,
...
NTSTATUS
NTAPI
ZwPrivilegeObjectAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                            _In_ OPTIONAL PVOID HandleId,
                            _In_ HANDLE ClientToken,
                            _In_ ACCESS_MASK DesiredAccess,
                            _In_ PPRIVILEGE_SET Privileges,
...
NTSTATUS
NTAPI
ZwCloseObjectAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                        _In_ OPTIONAL PVOID HandleId,
                        _In_ BOOLEAN GenerateOnClose);

NTSTATUS
NTAPI
ZwDeleteObjectAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                         _In_ OPTIONAL PVOID HandleId,
                         _In_ BOOLEAN GenerateOnClose);

NTSTATUS
NTAPI
ZwPrivilegedServiceAuditAlarm(_In_ PUNICODE_STRING SubsystemName,
                              _In_ PUNICODE_STRING ServiceName,
                              _In_ HANDLE ClientToken,
                              _In_ PPRIVILEGE_SET Privileges,
                              _In_ BOOLEAN AccessGranted);
...
NTSTATUS
NTAPI
ZwContinue(_In_ PCONTEXT ContextRecord, _In_ BOOLEAN TestAlert);

NTSTATUS
NTAPI
ZwRaiseException(_In_ PEXCEPTION_RECORD ExceptionRecord,
                 _In_ PCONTEXT ContextRecord, _In_ BOOLEAN FirstChance);

ULONG DbgPrint(_In_ PCH Format, ...);

VOID NTAPI DebugService2(PVOID Arg1, PVOID Arg2, ULONG Service);

__inline LARGE_INTEGER NTAPI RtlLargeIntegerAdd(LARGE_INTEGER Addend1,
                                                LARGE_INTEGER Addend2);

__inline LARGE_INTEGER NTAPI RtlEnlargedIntegerMultiply(LONG Multiplicand,
                                                        LONG Multiplier);

__inline LARGE_INTEGER NTAPI RtlEnlargedUnsignedMultiply(ULONG Multiplicand,
                                                         ULONG Multiplier);

__inline ULONG NTAPI RtlEnlargedUnsignedDivide(_In_ ULARGE_INTEGER Dividend,
                                               _In_ ULONG Divisor,
                                               _In_ PULONG Remainder OPTIONAL);

__inline LARGE_INTEGER NTAPI RtlLargeIntegerNegate(LARGE_INTEGER Subtrahend);

__inline LARGE_INTEGER NTAPI RtlLargeIntegerSubtract(LARGE_INTEGER Minuend,
                                                     LARGE_INTEGER Subtrahend);

LARGE_INTEGER
NTAPI
RtlExtendedMagicDivide(LARGE_INTEGER Dividend, LARGE_INTEGER MagicDivisor,
                       CCHAR         ShiftCount);
...
__inline LARGE_INTEGER NTAPI RtlConvertLongToLargeInteger(LONG SignedInteger);

__inline LARGE_INTEGER NTAPI
RtlConvertUlongToLargeInteger(ULONG UnsignedInteger);

__inline LARGE_INTEGER NTAPI
RtlLargeIntegerShiftLeft(LARGE_INTEGER LargeInteger, CCHAR ShiftCount);

__inline LARGE_INTEGER NTAPI
RtlLargeIntegerShiftRight(LARGE_INTEGER LargeInteger, CCHAR ShiftCount);

__inline LARGE_INTEGER NTAPI
RtlLargeIntegerArithmeticShift(LARGE_INTEGER LargeInteger, CCHAR ShiftCount);

__inline BOOLEAN NTAPI RtlCheckBit(PRTL_BITMAP BitMapHeader, ULONG BitPosition);

BOOLEAN
NTAPI
RtlIsValidOemCharacter(_In_ _Out_ PWCHAR Char);

PIMAGE_NT_HEADERS
NTAPI
RtlpImageNtHeader(PVOID Base);

RTL_PATH_TYPE RtlDetermineDosPathNameType_U(_In_ PCWSTR DosFileName);

PRTL_TRACE_DATABASE RtlTraceDatabaseCreate(_In_ ULONG Buckets, _In_ SIZE_T MaximumSize OPTIONAL,
                                           _In_ ULONG Flags, _In_ ULONG Tag,
                                           _In_ RTL_TRACE_HASH_FUNCTION HashFunction OPTIONAL);

BOOLEAN RtlTraceDatabaseValidate(_In_ PRTL_TRACE_DATABASE Database);

BOOLEAN RtlTraceDatabaseAdd(_In_ PRTL_TRACE_DATABASE Database, _In_ ULONG Count,
                            _In_ PVOID * Trace,
                            _Out_ PRTL_TRACE_BLOCK *TraceBlock OPTIONAL);

BOOLEAN RtlTraceDatabaseFind(PRTL_TRACE_DATABASE Database, _In_ ULONG Count,
                             _In_ PVOID *        Trace,
                             _Out_ PRTL_TRACE_BLOCK *TraceBlock OPTIONAL);

BOOLEAN RtlTraceDatabaseEnumerate(PRTL_TRACE_DATABASE Database,
                                  _Out_ PRTL_TRACE_ENUMERATE Enumerate,
                                  _Out_ PRTL_TRACE_BLOCK * TraceBlock);

VOID RtlTraceDatabaseLock(_In_ PRTL_TRACE_DATABASE Database);

VOID RtlTraceDatabaseUnlock(_In_ PRTL_TRACE_DATABASE Database);

VOID RtlpGetStackLimits(_Out_ PULONG_PTR LowLimit, _Out_ PULONG_PTR HighLimit);
...
NTSTATUS
NTAPI
RtlEXITCriticalSection(PRTL_CRITICAL_SECTION CriticalSection);
...
NTSTATUS
NTAPI
LdrLoadDll(_In_ OPTIONAL PWSTR DllPath, _In_ OPTIONAL PULONG DllCharacteristics,
...
NTSTATUS
NTAPI
LdrGetDllHandle(_In_ OPTIONAL PWSTR DllPath,
                _In_ OPTIONAL PULONG DllCharacteristics,
                _In_ PUNICODE_STRING DllName, _Out_ PVOID * DllHandle);

NTSTATUS
NTAPI
LdrGetDllHandleEx(_In_ ULONG Flags, _In_ OPTIONAL PCWSTR DllPath,
                  _In_ OPTIONAL PULONG DllCharacteristics,
...
NTSTATUS
NTAPI
LdrGetDllHandleByName(_In_ OPTIONAL PUNICODE_STRING BaseDllName,
...
NTSTATUS
NTAPI
LdrLockLoaderLock(_In_ ULONG Flags, _Out_ OPTIONAL ULONG * Disposition,
                  _Out_ PVOID *                            Cookie);

NTSTATUS
NTAPI
LdrRelocateImage(_In_ PVOID NewBase, _In_ PSTR LoaderName,
                 _In_ NTSTATUS Success, _In_ NTSTATUS Conflict,
                 _In_ NTSTATUS Invalid);

NTSTATUS
NTAPI
LdrRelocateImageWithBias(_In_ PVOID NewBase, _In_ LONGLONG Bias,
                         _In_ PSTR LoaderName, _In_ NTSTATUS Success,
                         _In_ NTSTATUS Conflict, _In_ NTSTATUS Invalid);
...
BOOLEAN
NTAPI
LdrVerifyMappedImageMatchesChecksum(_In_ PVOID BaseAddress,
                                    _In_ SIZE_T NumberOfBytes,
                                    _In_ ULONG FileLength);

NTSTATUS
NTAPI
LdrQueryModuleServiceTags(_In_ PVOID DllHandle, _Out_ PULONG ServiceTagBuffer,
...
LdrRegisterDllNotification(
	...
	_In_ PVOID Context, _Out_ PVOID * Cookie);
...
PVOID
NTAPI
A_SHAUpdate(_In_ _Out_ PSHA_CTX, _In_ PCHAR, _In_ UINT);

PVOID
NTAPI
A_SHAInit(_In_ _Out_ PSHA_CTX, _Out_ PVOID);

BOOLEAN
NTAPI
RtlDosPathNameToNtPathName_U(_In_ PCWSTR DosFileName,
                             _Out_ PUNICODE_STRING NtFileName,
                             _Out_ PWSTR *FilePart OPTIONAL, PVOID Reserved);
...
PVOID
NTAPI
RtlAddVectoredExceptionHandler(_In_ ULONG First,
                               _In_ PVECTORED_EXCEPTION_HANDLER Handler);

PVOID
NTAPI
RtlAddVectoredContinueHandler(_In_ ULONG First,
                              _In_ PVECTORED_EXCEPTION_HANDLER Handler);
...
PVOID RtlEncodePointer(PVOID Ptr);

PVOID RtlDecodePointer(PVOID Ptr);

PVOID RtlEncodeSystemPointer(PVOID Ptr);

PVOID RtlDecodeSystemPointer(PVOID Ptr);
...
NTAPI
BCryptGenRandom(
	_In_opt_ HANDLE hAlgorithm,
	_Out_ PUCHAR pbBuffer,
	_In_ ULONG cbBuffer,
	_In_ ULONG dwFlags
);

typedef NTSTATUS(NTAPI* BCryptGenRandom_t)(
	_In_opt_ HANDLE hAlgorithm,
	_Out_ PUCHAR pbBuffer,
	_In_ ULONG cbBuffer,
	_In_ ULONG dwFlags
...
typedef BOOLEAN(NTAPI* SystemFunction036_t)(
	_Out_ PVOID RandomBuffer,
	_In_ ULONG RandomBufferLength
);
...
typedef unsigned long (WINAPI *GetAdaptersInfo_t)(
	_Out_ PIP_ADAPTER_INFO pAdapterInfo,
	_Inout_ unsigned long* pOutBufLen
);

#define HASH_IPHLPAPI        0x26625329
#define HASH_GETADAPTERSINFO 0x27D1D38C
...
NTSTATUS
NTAPI
RtlSetLFHDebuggingInformation(PVOID                       LFHHeap,
...
NTSTATUS
NTAPI
RtlNewSecurityObjectEx(_In_ PSECURITY_DESCRIPTOR ParentDescriptor OPTIONAL,
                       _In_ PSECURITY_DESCRIPTOR CreatorDescriptor OPTIONAL,
                       _Out_ PSECURITY_DESCRIPTOR * NewDescriptor,
                       _In_ GUID *ObjectType OPTIONAL,
                       _In_ BOOLEAN IsDirectoryObject,
                       _In_ ULONG AutoInheritFlags, _In_ HANDLE Token,
                       _In_ PGENERIC_MAPPING GenericMapping);
...
RtlNewSecurityObjectWithMultipleInheritance(
	...
	_In_ PSECURITY_DESCRIPTOR CreatorDescriptor OPTIONAL,
	_Out_ PSECURITY_DESCRIPTOR * NewDescriptor, _In_ GUID **pObjectType OPTIONAL,
	_In_ ULONG GuidCount, _In_ BOOLEAN IsDirectoryObject,
	_In_ ULONG AutoInheritFlags, _In_ HANDLE Token,
	_In_ PGENERIC_MAPPING GenericMapping);
#if !defined(_WINDOWS_)
NTSTATUS
...
NTSTATUS
NTAPI
RtlQuerySecurityObject(PSECURITY_DESCRIPTOR ObjectDescriptor,
                       SECURITY_INFORMATION SecurityInformation,
                       PSECURITY_DESCRIPTOR ResultantDescriptor,
                       ULONG                DescriptorLength, PULONG ReturnLength);

NTSTATUS
NTAPI
RtlRegisterWait(_Out_ PHANDLE WaitHandle, _In_ HANDLE Handle,
                _In_ WAITORTIMERCALLBACKFUNC Function, _In_ PVOID Context,
                _In_ ULONG Milliseconds, _In_ ULONG Flags);
...
NTSTATUS
NTAPI
RtlSetIoCompletionCallback(_In_ HANDLE FileHandle,
                           _In_ APC_CALLBACK_FUNCTION CompletionProc,
                           _In_ ULONG Flags);

NTSTATUS
NTAPI
RtlSetSecurityObject(SECURITY_INFORMATION  SecurityInformation,
...
RtlSetSecurityObjectEx(
	...
	_In_ ULONG AutoInheritFlags, _In_ PGENERIC_MAPPING GenericMapping,
...
NTSTATUS RtlWow64EnableFsRedirection(_In_ BOOLEAN Wow64FsEnableRedirection);

NTSTATUS RtlWow64EnableFsRedirectionEx(_In_ PVOID Wow64FsEnableRedirection,
...
NTSTATUS
NTAPI
RtlDeregisterWait(_In_ HANDLE WaitHandle);

NTSTATUS
NTAPI
RtlDeregisterWaitEx(_In_ HANDLE WaitHandle, _In_ HANDLE Event);
...
typedef VOID (*PKNORMAL_ROUTINE)(_In_ PVOID NormalContext,
                                 _In_ PVOID SystemArgument1,
                                 _In_ PVOID SystemArgument2);

VOID KiUserCallbackDispatcher(_In_ ULONG ApiNumber, _In_ PVOID InputBuffer,
                              _In_ ULONG INputLength);

NTSTATUS
NTAPI
CsrClientConnectToServer(_In_ PWSTR ObjectDirectory, _In_ ULONG ServertDllIndex,
...
NTSTATUS
NTAPI
CsrClientCallServer(_In_ _Out_ PCSR_API_MSG m,
                    _In_ _Out_ PCSR_CAPTURE_HEADER CaptureBuffer OPTIONAL,
                    _In_ CSR_API_NUMBER ApiNumber, _In_ ULONG ArgLength);

PCSR_CAPTURE_HEADER
NTAPI
CsrAllocateCaptureBuffer(_In_ ULONG CountMessagePointers,
                         _In_ ULONG CountCapturePointers, _In_ ULONG Size);

VOID NTAPI CsrFreeCaptureBuffer(_In_ PCSR_CAPTURE_HEADER CaptureBuffer);

ULONG
NTAPI
CsrAllocateMessagePointer(_In_ _Out_ PCSR_CAPTURE_HEADER CaptureBuffer,
                          _In_ ULONG Length, _Out_ PVOID * Pointer);

VOID NTAPI CsrCaptureMessageBuffer(_In_ _Out_ PCSR_CAPTURE_HEADER CaptureBuffer,
                                   _In_ PVOID Buffer OPTIONAL,
                                   _In_ ULONG Length,
                                   _Out_ PVOID * CapturedBuffer);

VOID NTAPI CsrCaptureMessageString(_In_ _Out_ PCSR_CAPTURE_HEADER CaptureBuffer,
                                   _In_ PCSTR String, _In_ ULONG Length,
                                   _In_ ULONG MaximumLength,
                                   _Out_ PSTRING CapturedString);

PLARGE_INTEGER
NTAPI
CsrCaptureTimeout(_In_ ULONG Milliseconds, _Out_ PLARGE_INTEGER Timeout);

VOID NTAPI CsrProbeForWrite(_In_ PVOID Address, _In_ ULONG Length,
                            _In_ ULONG Alignment);

VOID NTAPI CsrProbeForRead(_In_ PVOID Address, _In_ ULONG Length,
                           _In_ ULONG Alignment);
...
NTSTATUS
NTAPI
CsrSetPriorityClass(_In_ HANDLE ProcessHandle, _In_ _Out_ PULONG PriorityClass);

NTSTATUS
NTAPI
RtlCreateProcessReflection(
	_In_ HANDLE ProcessHandle, _In_ ULONG Flags,
	_In_ OPTIONAL PVOID StartRoutine, _In_ OPTIONAL PVOID StartContext,
	_In_ OPTIONAL HANDLE EventHandle,
	_Out_ OPTIONAL PRTL_PROCESS_REFLECTION_INFORMATION ReflectionInformation);

NTSTATUS
NTAPI
RtlCloneUserProcess(_In_ ULONG ProcessFlags,
                    _In_ OPTIONAL PSECURITY_DESCRIPTOR
...
NTSTATUS
NTAPI
RtlQueryProcessModuleInformation(_In_ HANDLE hProcess OPTIONAL,
                                 _In_ ULONG Flags,
                                 _In_ _Out_ PRTL_DEBUG_INFORMATION Buffer);

NTSTATUS
NTAPI
RtlQueryProcessBackTraceInformation(_In_ _Out_ PRTL_DEBUG_INFORMATION Buffer);

NTSTATUS
NTAPI
RtlQueryProcessHeapInformation(_In_ _Out_ PRTL_DEBUG_INFORMATION Buffer);

NTSTATUS
NTAPI
RtlQueryProcessLockInformation(_In_ _Out_ PRTL_DEBUG_INFORMATION Buffer);

PRTL_DEBUG_INFORMATION
NTAPI
RtlCreateQueryDebugBuffer(_In_ ULONG MaximumCommit OPTIONAL,
                          _In_ BOOLEAN UseEventPair);
...
NTSTATUS
NTAPI
RtlCreateTimer(_In_ HANDLE TimerQueueHandle, _Out_ HANDLE * Handle,
               _In_ WAITORTIMERCALLBACKFUNC Function, _In_ PVOID Context,
               _In_ ULONG DueTime, _In_ ULONG Period, _In_ ULONG Flags);

NTSTATUS
NTAPI
RtlUpdateTimer(_In_ HANDLE TimerQueueHandle, _In_ HANDLE TimerHandle,
               _In_ ULONG DueTime, _In_ ULONG Period);

NTSTATUS
NTAPI
RtlDeleteTimer(_In_ HANDLE TimerQueueHandle, _In_ HANDLE TimerToCancel,
               _In_ HANDLE Event);

NTSTATUS
NTAPI
RtlDeleteTimerQueue(_In_ HANDLE TimerQueueHandle);

NTSTATUS
NTAPI
RtlDeleteTimerQueueEx(_In_ HANDLE TimerQueueHandle, _In_ HANDLE Event);
...
ULONG RtlDosSearchPath_U(_In_ PWSTR lpPath, _In_ PWSTR lpFileName,
                         _In_ PWSTR lpExtension OPTIONAL, _In_ ULONG nBufferLength,
                         _Out_ PWSTR lpBuffer, _Out_ PWSTR * lpFilePart);

void NTAPI RtlInitString(PSTRING DestinationString, PCSZ SourceString);

ULONG
NTAPI
RtlGetFullPathName_U(_In_ PCWSTR lpFileName, _In_ ULONG nBufferLength,
                     _Out_ PWSTR lpBuffer, _Out_ OPTIONAL PWSTR * lpFilePart);
...
ULONG
NTAPI
EtwWriteUMSecurityEvent(PCEVENT_DESCRIPTOR     EventDescriptor,
                        USHORT                 EventProperty, ULONG UserDataCount,
                        PEVENT_DATA_DESCRIPTOR UserData);

ULONG
NTAPI
EtwEventWriteEndScenario(REGHANDLE          RegHandle,
                         PCEVENT_DESCRIPTOR EventDescriptor,
                         ULONG              UserDataCount, PEVENT_DATA_DESCRIPTOR UserData);

ULONG
NTAPI
EtwEventWriteFull(REGHANDLE              RegHandle, PCEVENT_DESCRIPTOR EventDescriptor,
                  USHORT                 EventProperty, LPCGUID        ActivityId,
                  LPCGUID                RelatedActivityId, ULONG      UserDataCount,
                  PEVENT_DATA_DESCRIPTOR UserData);

ULONG
NTAPI
EtwEventWriteStartScenario(REGHANDLE              RegHandle,
                           PCEVENT_DESCRIPTOR     EventDescriptor,
                           ULONG                  UserDataCount,
                           PEVENT_DATA_DESCRIPTOR UserData);
...
NTSTATUS
NTAPI
NtCreateChannel(_Out_ PHANDLE ChannelHandle,
                _In_ POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL);

NTSTATUS
NTAPI
NtOpenChannel(_Out_ PHANDLE ChannelHandle,
              _In_ POBJECT_ATTRIBUTES ObjectAttributes);

NTSTATUS
NTAPI
NtListenChannel(_In_ HANDLE ChannelHandle, _Out_ PCHANNEL_MESSAGE * Message);

NTSTATUS
NTAPI
NtSendWaitReplyChannel(_In_ HANDLE ChannelHandle, _In_ PVOID Text,
                       _In_ ULONG Length, _Out_ PCHANNEL_MESSAGE * Message);

NTSTATUS
NTAPI
NtReplyWaitSendChannel(_In_ PVOID Text, _In_ ULONG Length,
...
void NTAPI
RtlUpdateClonedCriticalSection(PRTL_CRITICAL_SECTION CriticalSection);

NTSTATUS
NTAPI
RtlGetFullPathName_UstrEx(PUNICODE_STRING  FileName,
                          PUNICODE_STRING  StaticString,
                          PUNICODE_STRING  DynamicString,
                          PPUNICODE_STRING StringUsed, PULONG          FilePartPrefixCch,
                          PUCHAR           NameInvalid, PRTL_PATH_TYPE InputPathType,
...
NTSTATUS
NTAPI
ZwWow64QueryInformationProcess64(_In_ HANDLE ProcessHandle,
                                 _In_ PROCESSINFOCLASS ProcessInformationClass,
                                 _Out_ PVOID ProcessInformation,
                                 _In_ ULONG ProcessInformationLength,
                                 _Out_ OPTIONAL PULONG ReturnLength);
...
ZwWow64QueryVirtualMemory64(
	_In_ HANDLE ProcessHandle, _In_ PVOID BaseAddress,
	_In_ MEMORY_INFORMATION_CLASS MemoryInformationClass,
...
NTSTATUS
NTAPI
ZwWow64ReadVirtualMemory64(_In_ HANDLE ProcessHandle,
                           _In_ OPTIONAL PVOID BaseAddress, _Out_ PVOID Buffer,
                           _In_ SIZE_T BufferSize,
                           _Out_ OPTIONAL PSIZE_T NumberOfBytesRead);

NTSTATUS
NTAPI
ZwWow64WriteVirtualMemory64(_In_ HANDLE ProcessHandle,
                            _In_ OPTIONAL PVOID BaseAddress,
                            _In_ CONST VOID * Buffer, _In_ SIZE_T BufferSize,
                            _Out_ OPTIONAL PSIZE_T NumberOfBytesWritten);

void NTAPI
ZwWow64GetCurrentProcessorNumberEx(_Out_ PPROCESSOR_NUMBER ProcNumber);

PCSR_CAPTURE_HEADER
NTAPI
ZwWow64CsrAllocateCaptureBuffer(_In_ ULONG CountMessagePointers,
                                _In_ ULONG CountCapturePointers,
                                _In_ ULONG Size);

ULONG
NTAPI
ZwWow64CsrAllocateMessagePointer(_In_ _Out_ PCSR_CAPTURE_HEADER CaptureBuffer,
                                 _In_ ULONG Length, _Out_ PVOID * Pointer);

void NTAPI ZwWow64CsrCaptureMessageBuffer(
	_In_ _Out_ PCSR_CAPTURE_HEADER CaptureBuffer, _In_ PVOID Buffer OPTIONAL,
	_In_ ULONG Length, _Out_ PVOID * CapturedBuffer);

void NTAPI ZwWow64CsrCaptureMessageString(
	_In_ _Out_ PCSR_CAPTURE_HEADER CaptureBuffer, _In_ PCSTR String,
	_In_ ULONG Length, _In_ ULONG MaximumLength, _Out_ PSTRING CapturedString);

NTSTATUS
NTAPI
ZwWow64CsrClientConnectToServer(
	_In_ PWSTR ObjectDirectory, _In_ ULONG ServerDllIndex,
	_In_ PCSR_CALLBACK_INFO CallbackInformation OPTIONAL,
	_In_ PVOID ConnectionInformation,
	_In_ _Out_ PULONG ConnectionInformationLength OPTIONAL,
	_Out_ PBOOLEAN CalledFromServer OPTIONAL);

void NTAPI ZwWow64CsrFreeCaptureBuffer(_In_ PCSR_CAPTURE_HEADER CaptureBuffer);
...
NTSTATUS
NTAPI
ZwWow64DebuggerCall(_In_ ULONG ServiceClass, _In_ ULONG Arg1, _In_ ULONG Arg2);

NTSTATUS
NTAPI
RtlCleanUpTEBLangLists(void);

VOID KiUserApcDispatcher(PVOID NormalContext, PVOID              SystemArgument1,
                         PVOID SystemArgument2, PKNORMAL_ROUTINE NormalRoutine);
...
NTSTATUS
NTAPI
RtlQueueWorkItem(_In_ WORKERCALLBACKFUNC CallbackFunction,
                 _In_ OPTIONAL PVOID Context, _In_ ULONG Flags);

NTSTATUS
NTAPI
RtlCreateUserStack(SIZE_T       CommittedStackSize, SIZE_T MaximumStackSize,
                   SIZE_T       ZeroBits, ULONG            PageSize, ULONG ReserveAlignment,
...
NTSTATUS
NTAPI
LdrQueryProcessModuleInformation(PRTL_PROCESS_MODULES ModuleInformation,
...
IMPORT_FN wchar_t*__cdecl wcsncat(wchar_t* front, const wchar_t* back,
                                  size_t   count);
IMPORT_FN wchar_t*__cdecl wcsncpy(wchar_t* dest, const wchar_t* source,
                                  size_t   count);
#endif
#endif
#ifndef HINTERNET
...
#endif
#ifndef WINHTTP_ADDREQ_FLAG_ADD
#define WINHTTP_ADDREQ_FLAG_ADD 0x20000000
#endif
#ifndef WINHTTP_ADDREQ_FLAG_REPLACE
#define WINHTTP_ADDREQ_FLAG_REPLACE 0x80000000
#endif
typedef HINTERNET(WINAPI* WinHttpOpen_t)(LPCWSTR pszAgentW, DWORD dwAccessType,
                                         LPCWSTR pszProxyW,
...
typedef BOOL(WINAPI* WinHttpReceiveResponse_t)(HINTERNET hRequest,
                                               LPVOID    lpReserved);

typedef BOOL(WINAPI* WinHttpQueryDataAvailable_t)(
	HINTERNET hRequest, LPDWORD lpdwNumberOfBytesAvailable);

typedef BOOL(WINAPI* WinHttpReadData_t)(HINTERNET hRequest, LPVOID lpBuffer,
                                        DWORD     dwNumberOfBytesToRead,
                                        LPDWORD   lpdwNumberOfBytesRead);

typedef BOOL(WINAPI* WinHttpCloseHandle_t)(HINTERNET hInternet);

typedef BOOL(WINAPI* WinHttpAddRequestHeaders_t)(HINTERNET hRequest,
                                                 LPCWSTR   lpszHeaders,
                                                 DWORD     dwHeadersLength,
                                                 DWORD     dwModifiers);

typedef BOOL(WINAPI* WinHttpQueryHeaders_t)(HINTERNET hRequest,
                                            DWORD     dwInfoLevel, LPCWSTR pwszName,
                                            LPVOID    lpBuffer,
                                            LPDWORD   lpdwBufferLength,
                                            LPDWORD   lpdwIndex);

HINTERNET WINAPI WinHttpOpen(LPCWSTR pszAgentW, DWORD   dwAccessType,
                             LPCWSTR pszProxyW, LPCWSTR pszProxyBypassW,
                             DWORD   dwFlags);

HINTERNET WINAPI WinHttpConnect(HINTERNET     hSession, LPCWSTR  pszServerName,
                                INTERNET_PORT nServerPort, DWORD dwReserved);

HINTERNET WINAPI WinHttpOpenRequest(HINTERNET hConnect, LPCWSTR       pwszVerb,
                                    LPCWSTR   pwszObjectName, LPCWSTR pwszVersion,
                                    LPCWSTR   pwszReferrer,
                                    LPCWSTR*  ppwszAcceptTypes, DWORD dwFlags);

BOOL WINAPI WinHttpSendRequest(HINTERNET hRequest, LPCWSTR       lpszHeaders,
...
BOOL WINAPI WinHttpReadData(HINTERNET hRequest, LPVOID lpBuffer,
                            DWORD     dwNumberOfBytesToRead,
                            LPDWORD   lpdwNumberOfBytesRead);

BOOL WINAPI WinHttpCloseHandle(HINTERNET hInternet);

BOOL WINAPI WinHttpAddRequestHeaders(HINTERNET hRequest, LPCWSTR      lpszHeaders,
                                     DWORD     dwHeadersLength, DWORD dwModifiers);

BOOL WINAPI WinHttpQueryHeaders(HINTERNET hRequest, DWORD           dwInfoLevel,
                                LPCWSTR   pwszName, LPVOID          lpBuffer,
                                LPDWORD   lpdwBufferLength, LPDWORD lpdwIndex);

BOOL WINAPI WinHttpSetOption(HINTERNET hInternet, DWORD dwOption,
                             LPVOID    lpBuffer, DWORD  dwBufferLength);

BOOL WINAPI WinHttpSetTimeouts(HINTERNET hInternet, int       nResolveTimeout,
...
RtlLookupFunctionEntry(_In_ DWORD64 ControlPc, _Out_ PDWORD64 ImageBase,
...
extern "C"{
	NTSYSAPI NTSTATUS NTAPI BaseThreadInitThunk(
		_In_ BOOLEAN Unknown, _In_ LPTHREAD_START_ROUTINE StartAddress,
		_In_opt_ PVOID Parameter);

	NTSYSAPI NTSTATUS NTAPI RtlUserThreadStart(
		_In_ LPTHREAD_START_ROUTINE StartAddress, _In_opt_ PVOID Parameter);
}
#ifndef _UNKNWN_H_
#define _UNKNWN_H_
#ifndef SUCCEEDED
...
#endif
#ifndef TokenIsRestricted
#define TokenIsRestricted ((TOKEN_INFORMATION_CLASS)21)
#endif
#ifndef TokenSandBoxInert
#define TokenSandBoxInert ((TOKEN_INFORMATION_CLASS)15)
#endif
...
#define TH32CS_SNAPALL                                            \
  (TH32CS_SNAPHEAPLIST | TH32CS_SNAPPROCESS | TH32CS_SNAPTHREAD | \
   TH32CS_SNAPMODULE)
...
NTSTATUS
NTAPI
RtlpInitializeExtendedProcessAttributeList(
	_Out_ PPS_ATTRIBUTE_LIST AttributeList,
...
typedef struct API_SET_NAMESPACE {
	uint32_t Version;
	uint32_t Size;
	uint32_t Flags;
	uint32_t Count;
...
BOOL WINAPI Process32FirstW(_In_ HANDLE hSnapshot,
                            _Inout_ LPPROCESSENTRY32W lppe);
BOOL WINAPI Process32NextW(_In_ HANDLE hSnapshot, _Out_ LPPROCESSENTRY32W lppe);
...
BOOL WINAPI Module32FirstW(_In_ HANDLE hSnapshot,
                           _Inout_ LPMODULEENTRY32W lpme);
BOOL WINAPI Module32NextW(_In_ HANDLE hSnapshot, _Out_ LPMODULEENTRY32W lpme);
BOOL WINAPI Heap32ListFirst(_In_ HANDLE hSnapshot, _Inout_ LPHEAPLIST32 lphl);
BOOL WINAPI Heap32ListNext(_In_ HANDLE hSnapshot, _Out_ LPHEAPLIST32 lphl);
BOOL WINAPI Heap32First(_In_ LPHEAPENTRY32 lphe, _In_ DWORD th32ProcessID,
                        _In_ ULONG_PTR th32HeapID);
BOOL WINAPI Heap32Next(_Inout_ LPHEAPENTRY32 lphe);

BOOL WINAPI ConvertStringSidToSidA(_In_ LPCSTR StringSid, _Out_ PSID * Sid);

BOOL WINAPI ConvertStringSidToSidW(_In_ LPCWSTR StringSid, _Out_ PSID * Sid);

BOOL WINAPI LogonUserExExW(_In_ LPTSTR lpszUsername, _In_opt_ LPTSTR lpszDomain,
                           _In_opt_ LPTSTR lpszPassword, _In_ DWORD dwLogonType,
...
BOOL WINAPI CreateProcessA (LPCSTR, LPSTR, LPSECURITY_ATTRIBUTES, LPSECURITY_ATTRIBUTES, BOOL, DWORD, LPVOID, LPCSTR,
...
#define SE_GROUP_LOGON_ID 0x80000000L
...
HANDLE WINAPI GetCurrentThreadToken(VOID);
HANDLE WINAPI GetCurrentProcessToken(VOID);

HLOCAL WINAPI LocalAlloc(_In_ UINT uFlags, _In_ SIZE_T uBytes);
HLOCAL WINAPI LocalFree(_In_opt_ HLOCAL hMem);

NTSYSAPI PVOID NTAPI
AddVectoredExceptionHandler(ULONG First, PVECTORED_EXCEPTION_HANDLER Handler);

NTSYSAPI ULONG NTAPI RemoveVectoredExceptionHandler(PVOID Handle);

NTSYSAPI SIZE_T NTAPI VirtualQuery(LPCVOID                   lpAddress,
