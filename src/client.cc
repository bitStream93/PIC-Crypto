...
declfn auto instance::demo_sha256(_In_ instance* ctx) -> void {
	...
	const char* test_msg = symbol<const char *> ("Hello, Crypto");
...
declfn auto instance::demo_chacha20_poly1305(_In_ instance* ctx) -> void {
	...
	const char* plaintext = symbol<const char *> (";)");
	...
	if (!ciphertext || !decrypted) return;
...
declfn auto instance::recv_encrypted(
	...
	uint8_t* plaintext_buf = (uint8_t *)ctx->kernel32.HeapAlloc (hHeap, 8, MSG_BUFFER_SIZE);

	if (!buffer || !plaintext_buf) {
	...
	if (received != static_cast<int> (frame_len)) {
		...
		memory::secure_erase (plaintext_buf, MSG_BUFFER_SIZE);
		ctx->kernel32.HeapFree (hHeap, 0, buffer);
		ctx->kernel32.HeapFree (hHeap, 0, plaintext_buf);
		return -1;
	}
	...
	if (valid) {
		...
		memory::copy (message, plaintext_buf, copy_len);
		message[copy_len] = '\0';
		result_len = static_cast<int> (copy_len);
	}
...
declfn auto instance::start(_In_ void* args) -> void {
	...
	DPRINT ("=== Connecting to server on 127.0.0.1:%d ===\n", SERVER_PORT);
	...
	uintptr_t sock = ws2_32.socket (AF_INET, SOCK_STREAM, IPPROTO_TCP);
	...
	const char* SERVER_ADDR = symbol<const char *> ("127.0.0.1");
	memory::set (&addr, 0, sizeof(addr));
	addr.sin_family = AF_INET;
	addr.sin_addr.S_un.S_addr = ws2_32.inet_addr (SERVER_ADDR);
	addr.sin_port = ws2_32.htons (SERVER_PORT);

	result = ws2_32.connect (sock, reinterpret_cast<const struct sockaddr *> (&addr), sizeof(addr));
	...
	int     recvd = ws2_32.recv (sock, reinterpret_cast<char *> (server_pub), X25519_KEY_SIZE, 0);
	if (recvd != X25519_KEY_SIZE) {
		DPRINT ("Failed to receive server public key\n");
		goto cleanup;
	}

	print_hex (this, STRING ("Server public key"), server_pub, X25519_KEY_SIZE); {

		uint8_t client_pub[X25519_KEY_SIZE], client_priv[X25519_KEY_SIZE];
		x25519_generate_keypair (this, client_pub, client_priv);

		print_hex (this, STRING ("Client public key"), client_pub, X25519_KEY_SIZE);


		int sent = ws2_32.send (sock, reinterpret_cast<char *> (client_pub), X25519_KEY_SIZE, 0);
		if (sent != X25519_KEY_SIZE) {
			DPRINT ("Failed to send client public key\n");
			memory::secure_erase (client_priv, sizeof(client_priv));
			goto cleanup;
		}


		uint8_t shared_secret[X25519_KEY_SIZE];
		x25519_shared_secret (shared_secret, client_priv, server_pub);
		...
		uint64_t nonce_counter = 0x8000000000000000ULL;

		DPRINT ("Key exchange complete, waiting for messages...\n\n");



		while (is_running) {
			char message[MSG_BUFFER_SIZE];
			int  len = recv_encrypted (this, sock, session_key, message, sizeof(message));

			if (len <= 0) {
				DPRINT ("Failed to receive message or connection closed\n");
				break;
			}
			...
			if (memory::strcmp (message, symbol<const char *> ("END")) == 0) {
				DPRINT ("Received END, sending final echo and disconnecting\n");
				is_running = false;
			}


			DPRINT ("Echoing: \"%s\"\n\n", message);
			if (!send_encrypted (this, sock, session_key, nonce_base, &nonce_counter, message)) {
				DPRINT ("Failed to send echo\n");
				break;
			}
		}
...
cleanup: ws2_32.closesocket (sock);
	ws2_32.WSACleanup();

	DPRINT ("Client shutting down.\n");
